...
SizeResult run_size(int n, int iterations, int runs) {
  ...
  tanh_eigen(x.data(), out_eigen.data(), n, drive);
  tanh_scalar(x.data(), out_scalar.data(), n, drive);
...
int main(int argc, char** argv) {
  ...
  std::printf("{\n");
  std::printf("  \"benchmark\": \"transcendental_tanh_eigen_vs_scalar\",\n");
  std::printf("  \"runs\": %d,\n", runs);
  std::printf("  \"iterations_per_run_256\": %d,\n", iter_256);
  std::printf("  \"iterations_per_run_1024\": %d,\n", iter_1024);
  std::printf("  \"iterations_per_run_4096\": %d,\n", iter_4096);
  std::printf("  \"tanh_256_eigen_ms\": %.6f,\n", r256.eigen_ms);
  std::printf("  \"tanh_256_scalar_ms\": %.6f,\n", r256.scalar_ms);
  std::printf("  \"tanh_256_speedup_ratio\": %.6f,\n", r256.speedup);
  std::printf("  \"max_abs_diff_256\": %.6e,\n", static_cast<double>(r256.max_abs_diff));
  std::printf("  \"tanh_1024_eigen_ms\": %.6f,\n", r1024.eigen_ms);
  std::printf("  \"tanh_1024_scalar_ms\": %.6f,\n", r1024.scalar_ms);
  std::printf("  \"tanh_1024_speedup_ratio\": %.6f,\n", r1024.speedup);
  std::printf("  \"max_abs_diff_1024\": %.6e,\n", static_cast<double>(r1024.max_abs_diff));
  std::printf("  \"tanh_4096_eigen_ms\": %.6f,\n", r4096.eigen_ms);
  std::printf("  \"tanh_4096_scalar_ms\": %.6f,\n", r4096.scalar_ms);
  std::printf("  \"tanh_4096_speedup_ratio\": %.6f,\n", r4096.speedup);
