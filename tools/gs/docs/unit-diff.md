...
- **`default_disagreements`** — the machine holds one value across every instance of the parameter and it is not the row's `def`.
- **`defaults_one_row_cannot_express`** — the machine's instances disagree with **each other**, so no single `def` byte can be right for all of them. A statement about the row's shape rather than its value, and the reason it is reported apart: a part's power-on receive channel is its own block's, and one byte cannot say sixteen things.
