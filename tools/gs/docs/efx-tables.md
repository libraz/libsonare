...
`derive_efx_tables.py` is what closes that. It reads an archive of what one individual SC-8850 answered and writes down, once, what each byte is worth — the fourteen shared conversions, and which (type, slot) pairs the archive gives one to. The result is committed as `efx-tables.json` and rendered into `src/midi/synth/gs_efx_tables.h`, so a fresh clone builds against the tables without fetching anything.
...
**Where the spelling is the bare byte range, the address is part of the selector rather than a report line.** `00–7F` is the whole byte and is printed against 321 parameters that have nothing to do with each other, so it cannot pick a table on its own; for output level, output pan and effect balance the claim's single address is what is left to select on. The trade is stated rather than hidden: a slot of one of those types carrying the same quantity at some other address is invisible here instead of reported. Post gain and the splice window are selected the same way, since `00/01/02/03` and `00–04` are bare counts of states that name no table either. So is the equaliser's corner, printed `00/01` like every on/off switch; there the address also picks the table, since the low and the high corner bytes share the spelling and not the states.

**A class may carry claims that widen its reach and not its law**, under `reach_also_by`. The splice window's law is read on the two-voice pitch shifter; the feedback shifter's claim measured that its own mode byte picks the same five lengths, which says which table that type uses without measuring a second one. Each `map` row names the claim that reached it, and a slot reached by two claims stops the run, because the row's provenance would otherwise be the order they were listed in. What this does not do is carry a law onto a type no claim measured: the keyboard multi prints a shift mode as well, and it stays unreached.
...
- **The post-gain steps against the records.** Every step the claim's three sets read, against six decibels, carried under the table as `steps_read`. Reported rather than gated, like the rate tables: the worst step is 0.06 dB short, the third step on every set, which the claim itself names as unexplained, so a gate tight enough to see it would refuse the law its author stands by. What stops the run is reading no step at all.
- **The splice windows against both claims.** The law is five windows in the ratio 3 : 4 : 6 : 8 : 12 from 1024 steps of the unit's clock. The three windows the claim reads as whole step counts (1024, 2048, 4096) must equal the law's, and the widening claim's five means must hold the ratio to within its own repeats of 0.0209 ms; either failing stops the run. The second and fourth windows are 4096/3 and 8192/3 steps under the law, and the claim cannot tell those from the nearest whole count, a third of a clock step away, so the law is what is stored.
- **The corners against the claims that read them.** A corner class table is read off the model its claim names, and a model whose corner is not two states split between byte 0 and every other byte, or whose byte is not the address the class selects on, stops the run. The model's second state is what a slot standing in it returns for any byte from 1 up; the table carries it at byte 1 alone, since a byte past the list is not taken (below). The output tone pair's two corners must sit inside the band its claim publishes as every corner that fits, and the combination equaliser's low corner within one band of the twelfth-octave set of the standalone equaliser's first state, which is what its claim reads it as.
- **The state of every claim a class rests on.** Thirteen of the fourteen laws rest on a standing claim. The splice window's rests on a parked one, parked over whether the second and fourth windows are whole step counts, which is the sub-step question above; the one entry resting on it is listed. An entry resting on a **parked** claim is listed one entry at a time rather than one claim at a time: a claim is parked over a specific residual question, and whether that question is one these tables lean on is decided per entry.
...
Which types a pair is put on is the chain skeleton's decision, not these tables'. `measured_on_types` records where each was read, so a type the skeleton extends a pair to is visibly one no reading placed it on.

## Lists of states

`state_lists` holds every (type, slot) whose page prints a list of states — spelled `00/01`, `00/01/02` and so on, consecutive from nought — with the list's length; the header renders it as `kGsEfxStateLists`. It is not a conversion: its claim reads that a byte past the list is not taken and the slot keeps the state it was in, so the GS layer refuses the write rather than converting it. `00/7F` uses the same slash and is not read as a list, since its claim reads that spelling as a function of the byte with every value taken. Every slot the claim asked must be in the table at the length it asked it at, or the run stops; a claim that is not standing stops it too. The claim asked some slots of every list length the page prints and states the rule for all of them, so most rows rest on the rule and not on a reading of their own, and are flagged `approximate` for it.
