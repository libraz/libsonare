...
def format_report(program: int, pattern_name: str, rows: list[dict]) -> str:
    ...
    for row in rows:
        ...
        lines.append(
            f"  release   {rel_m:>7} ms  vs {rel_o:>7} ms   Δ {d['release_delta_ms']:+.1f} ms"
        )
        lines.append(
...
def _format_resonances(rows: list[dict]) -> list[str]:
    ...
    model_driven = [
        r for r in fixed_resonances(model_rows, recurrence_only=True) if r["on_partial_notes"]
    ]
    oracle_driven = [
        r for r in fixed_resonances(oracle_rows, recurrence_only=True) if r["on_partial_notes"]
    ]
    ...
    def lines(header: str, pairs) -> list[str]:
        ...
        for label, found in pairs:
            ...
            for r in found:
                ...
                on = (
                    f", on a partial for {r['on_partial_notes']} of them"
    ...
    if model or oracle:
        out += lines(
            "fixed resonances in the attack (same frequency across notes):",
    ...
    if model_driven or oracle_driven:
        out += lines(
            "driven at a fixed pitch (on nearly every note, partial or not):",
...
def run_compare(args: argparse.Namespace) -> int:
    ...
    for program in programs:
        ...
        oracle = obtain_oracle(
            ...
            total,
            SR,
            [n.start for n in pattern.notes],
        )
        model, room = match_model_room(
            model,
            oracle,
            SR,
            [(n.start, n.start + n.dur) for n in pattern.notes],
            args.room,
        ...
        report = (
            format_drum_report(pattern.name, rows)
            if pattern.percussive
...
def run_export_probe(args: argparse.Namespace) -> int:
    ...
    for program in programs:
        ...
        if pattern.percussive:
            ...
            print(
                f"   drum note {note} ({drum_name(note)}) on MIDI channel 10, kit "
            ...
            print(
                f"     voicematch.py compare --programs {program} --drum-note {note} "
                f"--pattern {pattern.name} --oracle-wav <rendered.wav>"
            )
            continue
        print(
            f"   program {program} ({gm_name(program)}), pattern '{pattern.name}', "
            f"{total:.2f}s, {len(pattern.notes)} notes"
        )
        print(f"   Render it at {SR} Hz with the reference instrument, then pass the WAV back:")
        print(
            f"     voicematch.py compare --programs {program} --pattern {pattern.name} "
...
def run_room_match(args: argparse.Namespace) -> int:
    ...
    print(
        f"oracle room: RT60 {target.rt60_s:.2f}s  tail level {target.tail_db:+.1f}dB  "
    ...
    if target.gated():
        print(
            f"-> refusing: the probe holds each note for {target.note_window_s * 1000:.0f} ms "
    ...
    print()
    print(
        f"closest match: CC91 {result['cc91']}, reverb_decay {result['reverb_decay']} "
    ...
    if result["residual"] > 1.5:
        print(
            "  NOTE: residual above 1.5 — libsonare's tank cannot reach this space; "
...
def main() -> int:
    ...
    cmp_p.add_argument(
        "--programs", required=True, help="GM programs: '40', '40-43', or '40,42,71'"
    )
    ...
    cmp_p.add_argument(
        "--velocities", default="", help="override probe velocities, e.g. '64,100,127'"
    )
    cmp_p.add_argument(
        "--drum-note",
        type=int,
    ...
    exp_p.add_argument(
        ...
        default=None,
        dest="drum_note",
        help="export a drum-channel probe of this percussion instrument instead of a melodic one",
    )
    exp_p.add_argument(
        "--drum-gate-ms", type=int, default=0, dest="drum_gate_ms", help=DRUM_GATE_HELP
    )
