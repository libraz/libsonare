...
- **Agreement across the compass**, where the probe is a grid of one note per render (`profile.py measure`). A room is common to every note and an instrument's own decay is not, so a space measured on a minority of the notes is refused: the electric guitar's DI reports a credible 1.17 s over its whole grid at once, and note by note it is 3.56 s on one note of ten and nothing on the other nine. None of the guards above catch that one, since the length is believable and the notes are held for seconds.

An external WAV is measured, and so is a captured corpus — `profile.py measure` records what it found per timbre under `rooms` in the reference profile, and `compare` places the model in it. The fluidsynth route renders with its effect units off, so anything measured there is the instrument.
