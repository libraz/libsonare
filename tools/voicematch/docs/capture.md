...
**`source_class: module` together with `room: present` is a contradiction and does fail.** The module is captured with reverb, chorus, delay and the insertion effect all off, so one of the two is wrong — a module left with its effects on, or a capture from something else labelled as one — and a capture carrying the module's own reverb drives `room.py` into matching a tank rather than a building. `room` absent is unclassified rather than a clash: nobody has answered, which is a gap. The measured half of the same question — a module capture whose corpus `Room.is_dry()` says is not dry — is not wired yet and belongs with the first module capture, where it can be checked against audio instead of asserted.
