...
| `timbres[]` | `id`, `label`, and `slot_channel` — the slot, from `--probe-slots`. `channel` is the other quantity and is written only where a note number selects an instrument | — / `channel: 1` |
