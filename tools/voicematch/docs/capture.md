...
| `timbres[]` | `id`, `label`, and `slot_channel` — the slot, from `--probe-slots`. `channel` is the other quantity and is written only where a note number selects an instrument. `key_offset` and `key_map` are below; `keyswitch` is identity and belongs in the overlay | — / `channel: 1` |
...
| `source_class` | what kind of product answered this capture — `module`, `dedicated` or `library`, and absent means unclassified. A classification rather than an identity, so it is tracked while the product stays in the overlay — see below | `"unclassified"` |
...
**`source_class` is the same shape of answer about the product rather than about the signal chain.** [policy.json](../policy.json) aims a slot at one of two things: a slot naming a real instrument takes a modern recording, and a slot naming a sound the machine invented takes the machine, because nothing stands behind it for a recording to be made of. Which of those a capture actually answered is not in the audio and not in any tracked file unless it is written down — the product lives in the untracked overlay on purpose, so a pad aimed at the machine and fitted against a general library's idea of a warm pad passes every gate, every test and every audit, and reads as finished. Three classes: `module` is the machine itself, `dedicated` is a product covering one instrument, `library` is a general one spanning the map. **Absent means unclassified, never a default**, for exactly the reason `rig` gives above.

**It is a classification and never an identity.** `module` / `dedicated` / `library` names no product, which is what lets it sit in the tracked definition while `plugin` and every preset stay in the overlay. `tests/conformance/check_bank_policy.py` holds the values to that set and counts, per `make conformance` run, what answered each of the machine-defined slots.

**The machine-defined count is a ratchet: it may fall and it may not rise.** The slots answered by something other than the module are carried rather than failed on, because acquiring a module reference per slot is open-ended work against hardware and a red build nobody can clear is the shape the bank's rules refuse everywhere else. A *new* one is clearable, by whoever added the capture, and that is what goes red. Capturing one from the module lowers the count, and leaving the recorded number above it is reported too — a ratchet left above what the tree reaches is where the next wrong answer hides, the same reason a parity allowlist entry expires with its divergence.

**`source_class: module` together with `room: present` is a contradiction and does fail.** The module is captured with reverb, chorus, delay and the insertion effect all off, so one of the two is wrong — a module left with its effects on, or a capture from something else labelled as one — and a capture carrying the module's own reverb drives `room.py` into matching a tank rather than a building. `room` absent is unclassified rather than a clash: nobody has answered, which is a gap. **The measured half of the same question is `import_sf2.check_module_is_dry`**, which runs after every module import and fails on a corpus whose own note tails measure a space. It measures against `unclassified` rather than against what the capture declared, because the declaration is the thing under test: passing `room: none` through would make `measure_rooms` return before reading any audio and print that no space was found, which is a pass meaning the measurement never ran. A capture declaring `room: none` over a wet recording is exactly the case two declarations cannot catch.
...
**`key_map` is `key_offset`'s general form, for a mapping that is neither chromatic nor complete — and the two are exclusive.** A constant covers an instrument transposed bodily; it cannot cover one whose reachable pitches are a set. Natural harmonics are the case that forces it: a sampled guitar's harmonic timbre answers some thirty discrete pitches across forty-eight keys, several keys sound the same pitch on different strings, and the steps between them are the string tuning rather than a scale. So `timbres[].key_map` is written as an object keyed by the **sounding** note — `{"40": 5, "47": 7}` — which is the direction every caller asks in, makes a duplicate impossible to write, and puts the choice between two keys that reach one pitch where it belongs: in the capture, argued in a `_key_map` note beside it. A grid note no key can sound is refused when the definition loads rather than one note at a time during the run, and `AuSource.key` raises on an unmapped note instead of falling back to the offset — the fallback would send key 45 for note 45 and record whatever answered under note 45's label, which is a grid measuring notes the instrument never played. Establish the map by rendering every key and reading the pitch back, not by deriving it from the tuning: the same fingering appears at several keys and only the render says which one the library actually mapped.
...
## Importing a SoundFont instead of playing a plugin (`import_sf2.py`)

`capture.py corpus` records a plugin by playing it. `import_sf2.py` records a SoundFont by reading it, writing the same manifest into the same scratch root, so `profile`, `compare`, `autofit` and `diagnose` consume the result without knowing which produced it. The capture definition is the same pair of files: the tracked `<id>.json` holds the method, and the untracked `<id>.local.json` supplies `soundfont_dir` and, per timbre, `sf2` (the file) and `preset` (the tone name). A font full of one product's recordings *is* that product, so its name belongs in the half that is not published — the same rule the plugin triple follows, for the same reason.

**A SoundFont is read as a corpus of recordings and never played.** A SoundFont holds recorded audio plus the generators a player applies to it; `sf2.require_corpus` walks every generator the file sets and refuses one that would change what a note sounds like — a filter, an attenuation, a tuning, a loop, a modulation destination, a velocity split, or a volume envelope that moves the note inside the window a metric is read from. The alternative is worse than a refusal: running the samples through a partial player would make the reference *this importer's guess* at what the source produced, and no metric downstream could tell that apart from a measurement. Generators that cannot change one note on their own — the sample-address offsets, an LFO rate with nothing routed to it, the drum mute group — are ignored rather than refused, and `--report` prints the whole tally for a font that will not import.

Three further refusals, each preventing something silent:

- **A note is extracted only where the font holds a sample recorded at that pitch.** A player would stretch the neighbour, and a capture that did so would report a resampled E as a measurement of E. A note with no sample of its own is absent from the grid and counted in the manifest's `reach`.
- **A velocity axis is never invented.** A font with no `velRange` holds one recording per note at a velocity it does not state, so a grid asking for more than one velocity is refused rather than written out as identical audio in every row — which reads as a perfectly flat dynamic response rather than as a missing measurement. Declare one velocity and excuse `vel_range` and `dynamics` in `dimensions_na` with that as the reason.
- **A gate longer than the shortest recording is refused**, naming the longest the font can serve. A gate is what the analysis window is promised to be, and nothing here pads a recording to fill one.

**`reach` is part of the manifest, not a line of output.** It carries how many cells the grid asked for, how many were extracted, and which notes each timbre could not answer. A screening run reporting no divergence on a program has to be readable against how much of that program was ever compared; a fixed note grid makes "the font could not express it" and "there is nothing there" the same silence otherwise.

**`preroll_ms` must be `0`.** An extracted recording begins at its own first frame, so there is no lead-in — and a capture declaring one would have every consumer that strips a preroll cut that much off the attack. A key-switch lead of zero fits inside a zero preroll, which is what makes this legal at all.
...
**One of the dimensions has to be able to see gain, and the requirement is on the set of captures that gate a voice together rather than on any one of them.** Every other column is normalised — a band profile against its own loudest band, a crest against its own RMS, a decay against its own peak — which is what makes them measure timbre and also what makes all of them blind to output level. Rewriting eighteen of the kit's levels moved not one of them by a digit. `level` is the per-hit `peak_dbfs` against the reference's, and `vel_range` is not a substitute: it is a span, and a span cancels an offset by construction. So a voice split across captures moves `level` with the split rather than leaving it where the colour went: whichever capture takes it has to carry the bound, or nothing anywhere is watching that voice's gain. On the kit it is taken by `drums_module_hit`, the grid that reads every piece at one window.
