...
**The page says which kind of source answered the slot, and whether that is the kind the slot is aimed at.** `serve.py` resolves it per request from the capture's `source_class` and `policy.json`'s `reference_layer` rather than from anything written into a render, so a page rendered months ago answers against today's policy and a re-render buys nothing. Where the two disagree the line says so — a slot naming a sound the machine invented has nothing outside the machine to be recorded, and a library's idea of one sounds exactly like a finished voice. A slot `no_reference` names is reported as held to have none, with the name the machine gives the address, rather than as one nobody has captured.
...
**What was heard is said on the page and lands beside the renders.** The panel under the pictures narrows two answers at a time, in a listener's own words rather than in the names of the constants underneath — the first fork is whether it is the right instrument at all, which is the size of error the bank still carries — and writes one JSON line per note to `<scratch root>/feedback/<slug>.jsonl`, carrying what was sounding when it was written. The verdict is recorded apart from the finer tag, because *recognisably the instrument and I would still change it* is where most of the bank sits and is not the same finding as *this is a different instrument*. A kit is versioned per drum note rather than as a kit, so a note on one is attributed to the `dNNN` the strike it was taken on held. `tools/audition/heard.py` reads the log back and dates each note against when its unit last moved. The log is untracked for the same reason the renders are, so it is gone on a `make clean` and exists in no other clone.

The rest of the page — the transport, the two roles and their colours, blind mode, the addresses — is [`tools/audition/README.md`](../../audition/README.md).
