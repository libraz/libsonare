...
**A timbre's `channel` says what its note numbers MEAN; `slot_channel` says where the rack keeps it.** Channel 10 is what makes a note number select an instrument rather than a pitch, and `profile.is_percussion` reads it to choose the metric set for the whole capture. The two coincide right up until a rack holds a kit somewhere other than slot 10, and then no single field can serve: addressing the slot on 10 records whatever else lives there, and declaring the kit on 15 sends a drum map through the pitched measurements and reports it as a successful capture. They part company the other way too, and that one had already happened: a slot number written as `channel` claims a meaning it was never about, and the five melodic instruments the rack keeps in slot 10 were measured as drum maps. So a rack capture declares `slot_channel` and leaves `channel` alone — in the tracked definition, where the layout it was captured against is reviewable, with the untracked overlay free to override it for an installation arranged differently. Absent, `channel` addresses the slot, which `load_config` keeps safe by holding it to 1 or 10: the fallback can only ever reach the plugin that is not a rack, or the rack that does keep its kit where GM puts it.
...
**`rig --against` names sibling captures, and siblings from the same rack are the argument.** They came through one recording chain in one session, so what they cannot explain is the rig. It also reads a timbre of the subject's own corpus, which is how the model's grid is measured with the instrument that classified the reference: `--timbre model --against <same id>` puts the two rows side by side, and `render-grid --no-rig` adds the third row.

**A one-sided reading names a chain, never a stage.** The model's row is the voice through its rig, so an excess against the reference belongs to neither half until something separates them — the first run of this attributed program 30's to the cabinet and was wrong twice over. The three bound rigs share one cabinet, so swapping the preset for another that uses it moved the error by 0.25 dB; drive moved the far top and left the 5 kHz band inside half a dB across its whole range. What the excess was is the preset's tone stack, which put program 30's 5 kHz band 12.6 dB over its reference where program 29's sat 3.6 out. Binding 29's amplifier at 30's own gain halved it. Even that is an attribution within the model, not between the model and the reference: the reference's own split needs a direct capture of the same instrument, which is what a rig-switchable product is for.

**`takes` on a wet capture places the model in the reference's room first.** The model always renders dry — `write_smf` writes CC91 0 — so against a reference recorded in its building every tail figure would be a dry signal read against a wet one, and all of them would land outside the references' spread for that reason and not because the voice is wrong. Measured on the church organ before the correction: the tail fell at 78 dB/s against the references' 15 to 17, and its four band levels missed by 11 to 25 dB; after it, the decay is 23 dB/s and the tail tonality is inside their spread. A capture's own `dry` and `room` answers together decide whether the correction runs at all — nothing in the audio can tell a building from an instrument's own long release, and guessing wrong convolves an invented room onto every figure. **Both are needed and `dry` alone is not enough**: a plugin advertising no effect section is written `dry: false` for want of anything to switch off, which is 57 of the captures here, so reading dryness by itself put a room on every one of them. Four had one applied — the sustained voices, whose release is long enough to survive `is_dry()` — and each declares `room: "none"` on a measurement that says otherwise.
...
Both the spec sweep and the census address a patch through one map, `patch_addresses`: each patch once, at bank 0 where it has one and at its variation address otherwise. Thirty patches are GS variations with no bank-0 program, and rendering one at bank 0 sounds the capital tone instead — so an override scoped to the variation moves nothing, and the patch reads as having no live field. Sharing the map is the point rather than a tidiness: the census missed those thirty for a generation while the spec sweep had the same hole one function away, and a spec scoped to a variation is skipped by a run that still exits 0. A spec that names an *engine* constant rather than a patch is a different case — the constant is shared by every patch on that engine, so the stand-in prefers a capital tone and takes a variation only for an engine that has no bank-0 patch at all.
