...
**A band can stop discriminating** (`measure_band_edge`, one reference against itself). What marks that edge is not where the energy stops — a capture rolling off still has energy above it — but where a crash, a cowbell, a closed hi-hat and a cabasa stop reading tens of dB apart, because what is left is one shared transfer function rather than four instruments. A level test cannot find the boundary: on `reference/drums.json` 57 % of rows are still above the band floor at 8 kHz and 36 % at 10 kHz, so a floor count puts the edge wherever its threshold was chosen.

**Or it can discriminate and still hold no target** (`measure_agreement_edge`, two references against each other). A reference can separate its own instruments perfectly while sitting tens of dB from where they are, and every instrument being wrong by the same amount is what a recording chain does — so the band passes the spread test and the model is fitted to the chain. The separator is which part of the difference the two references share: they differ because they are different instruments, which scatters the difference from piece to piece, and because they are different chains, which shifts all of it one way. The scatter is the across-instrument IQR and the shift is the median, so a band whose median stands over its IQR is reporting the chains. Needs two recordings and cannot be faked from one, so most captures get no agreement edge at all.

The drum capture is why the second exists. Both of its references discriminate to 8 kHz, and they stop agreeing at 5: the median offset between them wanders between −9 and +6 dB from 50 Hz to 4 kHz against an IQR held near 12, then runs to −11.4, −17.5 and −24.4 dB at 5, 6.3 and 8 kHz while the IQR does not move. One of them is band-limited to about 11 kHz — its content at 14 kHz sits 97 to 122 dB under its own peak, which is removal rather than roll-off — and the bands under that ceiling carry its anti-alias skirt as if it were the kit. Both references come from the same plugin and the same capture path, which is what rules out the harness.
