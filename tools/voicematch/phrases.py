...
def piano_takes(program: int = 0) -> list[Take]:
    ...
    out.append(
        Take(
            "dynamics-c4",
            "Dynamics — C4 at five velocities",
            "one note at a time",
    ...
    out.append(
        Take(
            "chord-sustained",
            "Chord — C major triad held",
            "notes together",
            "C3 E3 G3, vel 88, 6 s",
            [Note(n, 88, 0.3, 6.0) for n in (48, 52, 55)],
            tail_s=4.0,
        )
    )
    ...
    out.append(
        Take(
            "arpeggio-legato",
            "Arpeggio — overlapping, no pedal",
            "notes together",
            "C3 E3 G3 C4 E4 G4 C5, each held past the next",
            [Note(n, 84, 0.3 + i * 0.42, 1.9) for i, n in enumerate((48, 52, 55, 60, 64, 67, 72))],
            tail_s=4.0,
        )
    )
    ...
    out.append(
        Take(
            "pedal-resonance",
            "Pedal — staccato notes under a held pedal",
            "the pedal",
    ...
    out.append(
        Take(
            "repeated-note",
            "Repeated note — eight strikes on a ringing string",
            "the pedal",
            "C4 vel 100, 190 ms apart",
            [Note(60, 100, 0.3 + i * 0.19, 0.14) for i in range(8)],
            tail_s=4.0,
        )
    )
    ...
    out.append(
        Take(
            "phrase-ballad",
            "Phrase — melody over a bass line, pedalled",
            "a phrase",
            "with the pedal changed on each bass note",
            [Note(n, v, 0.4 + s, d) for n, v, s, d in phrase],
            tail_s=5.0,
            cc_events=(
                (0.45, PEDAL, 127),
                (2.05, PEDAL, 0),
...
def harpsichord_takes(program: int = 6) -> list[Take]:
    ...
    out.append(
        Take(
            "register-sweep",
            "Register - the whole compass",
            "one note at a time",
            "F1 C2 C3 C4 C5 C6 F6, vel 96",
            [Note(n, 96, 0.3 + i * 1.5, 1.2) for i, n in enumerate((29, 36, 48, 60, 72, 84, 89))],
            tail_s=3.5,
        )
    )
    ...
    out.append(
        Take(
            "treble-sustain",
            "Treble - the top note held four seconds",
            "one note at a time",
    ...
    out.append(
        Take(
            ...
            "notes together",
            "C3 E3 G3, vel 88, 5 s",
            [Note(n, 88, 0.3, 5.0) for n in (48, 52, 55)],
            tail_s=3.5,
        )
    )
    ...
    out.append(
        Take(
            ...
            "notes together",
            "C3 E3 G3 C4 E4 G4 C5, each held past the next",
            [Note(n, 84, 0.3 + i * 0.34, 1.5) for i, n in enumerate((48, 52, 55, 60, 64, 67, 72))],
            tail_s=3.5,
        )
    )
...
def drum_takes(program: int = 0) -> list[Take]:
    ...
    out.append(
        Take(
            ...
            [
                Note(46, 110, 0.3, 0.05),
                Note(42, 100, 0.8, 0.05),
                Note(46, 110, 1.8, 0.05),
                Note(44, 90, 2.3, 0.05),
            ],
            tail_s=4.0,
    ...
    out.append(
        Take(
            "hh-sixteenths",
            "Hi-hat — sixteenths at 120",
            "at speed",
            "42 closed, accented on the beat",
            [Note(42, 112 if i % 4 == 0 else 96, 0.3 + i * 0.125, 0.05) for i in range(16)],
            tail_s=2.5,
            channel=DRUM_CHANNEL,
        )
    )
    ...
    out.append(
        Take(
            "snare-flam",
            "Snare — flams and a roll",
            "at speed",
            "grace note 28 ms ahead, then a 12-stroke roll",
            [
                Note(38, 60, 0.3, 0.05),
                Note(38, 110, 0.328, 0.05),
                Note(38, 60, 1.3, 0.05),
                Note(38, 110, 1.328, 0.05),
            ]
            + [Note(38, 90, 2.4 + i * 0.06, 0.05) for i in range(12)],
            tail_s=3.0,
            channel=DRUM_CHANNEL,
        )
    )
    ...
    out.append(
        Take(
            "tom-fill",
            "Toms — a descending fill",
            "the kit as a kit",
            "43 41 50 48 47 45, the capture's measured pitch order, high to low",
            [Note(n, 104, 0.3 + i * 0.22, 0.05) for i, n in enumerate((43, 41, 50, 48, 47, 45))],
            tail_s=3.5,
            channel=DRUM_CHANNEL,
        )
    )
    ...
    out.append(
        Take(
            ...
            + [
                Note(73, 104, 2.5, 0.05),
                Note(74, 104, 3.0, 0.05),
                Note(78, 100, 3.9, 0.05),
                Note(79, 100, 4.5, 0.05),
                Note(58, 112, 5.3, 0.05),
            ],
            tail_s=4.0,
            channel=DRUM_CHANNEL,
        )
    ...
    out.append(
        Take(
            "cymbal-wash",
            "Cymbals — the plates, let ring",
            "the long tail",
            "49 crash, 57 crash 2, 52 china, ten seconds",
            [Note(49, 120, 0.3, 0.05), Note(57, 120, 3.5, 0.05), Note(52, 112, 6.7, 0.05)],
            tail_s=10.0,
            channel=DRUM_CHANNEL,
        )
    )
    ...
    out.append(
        Take(
            "cymbal-ride",
            "Cymbals — rides, bell and splash",
            "the long tail",
            "51 ride, 59 ride 2, 53 bell, 55 splash",
            [Note(n, 100, 0.3 + i * 2.0, 0.05) for i, n in enumerate((51, 59, 53, 55))],
            tail_s=6.0,
            channel=DRUM_CHANNEL,
        )
    )
    ...
    out.append(
        Take(
            ...
            + [Note(38, 108, 0.9, 0.05), Note(38, 108, 2.1, 0.05)],
            tail_s=3.5,
            channel=DRUM_CHANNEL,
        )
    )
...
def sustained_takes(program: int = 40) -> list[Take]:
    ...
    out.append(
        Take(
            "swell",
            "Swell — one note from a whisper and back",
            "one note at a time",
            "expression from 16 to 127 and back over 6 s",
            [Note(mid, 100, 0.3, 6.0)],
            tail_s=2.5,
            cc_events=tuple(
                (0.3 + i * 0.25, EXPRESSION, v)
                for i, v in enumerate(list(range(16, 128, 11)) + list(range(127, 15, -11)))
            ),
        )
    )
    ...
    out.append(
        Take(
            "tongued",
            "Repeated notes — separated",
            "between notes",
            "the same pitch eight times, each articulated",
            [Note(mid, 100, 0.3 + i * 0.35, 0.22) for i in range(8)],
            tail_s=2.5,
        )
    )
    ...
    out.append(
        Take(
            "leap",
            "Leaps — across the compass",
            "between notes",
            "low, high, low, slurred",
            [Note(n, 96, 0.3 + i * 0.6, 0.7) for i, n in enumerate((lo, hi, mid, hi, lo))],
            tail_s=3.0,
        )
    )
    return out
...
def struck_takes(program: int = 0) -> list[Take]:
    ...
    out.append(
        Take(
            "register-sweep",
            "Register — across the compass",
            "one note at a time",
            "low, low-mid, mid, high-mid, high, vel 96",
            [
                Note(n, 96, 0.3 + i * 2.2, 1.8)
                for i, n in enumerate((lo, (lo + mid) // 2, mid, (mid + hi) // 2, hi))
            ],
            tail_s=4.0,
        )
    )
    ...
    out.append(
        Take(
            "chord-sustained",
            "Chord — a major triad held",
            "notes together",
            "root, third, fifth at vel 88, 6 s",
            [Note(mid + d, 88, 0.3, 6.0) for d in (0, 4, 7)],
            tail_s=4.0,
        )
    )
    ...
    out.append(
        Take(
            ...
            [Note(mid, 100, 0.3 + i * 0.19, 0.14) for i in range(8)],
            tail_s=4.0,
        )
    )
    ...
    out.append(
        Take(
            "phrase",
            "Phrase — melody over a bass line",
            "a phrase",
            "two hands, nothing held by a pedal",
            [
                Note(mid + d, v, 0.4 + s, dur)
                for d, v, s, dur in (
                    (0, 100, 0.00, 0.85),
                    (4, 88, 0.85, 0.85),
                    (7, 92, 1.70, 0.85),
...
def plucked_takes(program: int = 24) -> list[Take]:
    ...
    out.append(
        Take(
            "strum",
            "Chord — strummed, not struck",
            "notes together",
            "five strings 25 ms apart, let ring",
            [Note(mid + d, 92, 0.3 + i * 0.025, 3.0) for i, d in enumerate((0, 7, 12, 16, 19))],
            tail_s=3.5,
        )
    )
    ...
    out.append(
        Take(
            "arpeggio-legato",
            "Arpeggio — overlapping, all ringing",
            "notes together",
    ...
    out.append(
        Take(
            "repeated-note",
            "Repeated note — re-plucked while ringing",
            "at speed",
            "vel 100, 160 ms apart, eight times",
            [Note(mid, 100, 0.3 + i * 0.16, 0.12) for i in range(8)],
            tail_s=3.0,
        )
    )
    ...
    out.append(
        Take(
            "phrase",
            "Phrase — a line over open strings",
            "a phrase",
...
def modal_takes(program: int = 11) -> list[Take]:
    ...
    out.append(
        Take(
            "interval",
            "Two bars together — a third",
            "notes together",
            "struck at once, let ring",
            [Note(mid + d, 96, 0.3, 0.2) for d in (0, 4)],
            tail_s=8.0,
        )
    )
    ...
    out.append(
        Take(
            "roll",
            "Roll — sixteen strokes on one bar",
            "at speed",
            "80 ms apart, alternating accents",
            [Note(mid, 104 if i % 2 == 0 else 88, 0.3 + i * 0.08, 0.06) for i in range(16)],
            tail_s=5.0,
        )
    )
    ...
    out.append(
        Take(
            "phrase",
            "Phrase — a line with the bars left ringing",
            "a phrase",
            "nothing damped, so every note is heard against the last four",
            [
                Note(mid + d, v, 0.4 + s, 0.2)
                for d, v, s in (
                    (0, 96, 0.00),
                    (4, 88, 0.35),
                    (7, 92, 0.70),
...
def noise_takes(program: int = 120) -> list[Take]:
    ...
    out.append(
        Take(
            "single",
            "One trigger — heard whole",
            "one at a time",
            "held two seconds, then left to finish",
            [Note(mid, 100, 0.3, 2.0)],
            tail_s=6.0,
        )
    )
    ...
    out.append(
        Take(
            "retrigger",
            "Retriggered — before it has finished",
            "at speed",
            "six triggers 400 ms apart",
            [Note(mid, 100, 0.3 + i * 0.4, 0.3) for i in range(6)],
            tail_s=5.0,
        )
    )
    return out
...
def music_take(program: int, *, excerpt: str = "") -> Take | None:
    ...
    notes = [
        Note(n["pitch"] + shift, n["velocity"], n["start"], n["duration"]) for n in data["notes"]
    ]
    sub = data["note"]
    if shift:
