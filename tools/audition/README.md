...
`listen` in the header is one of two views. `bank` is the other: every GM program and GS variation the library voices, whatever has been rendered — the engine answering it, how far its calibration has got, and whether an oracle has been captured for it at all.
...
- **The toolbar narrows** by synthesis method, by whether an oracle exists, by whether anything is unwritten, and by name, engine or patch. Sorting by stage puts the worked-on voices at the top; the GM family headings belong to address order and come off under any other sort.
...
Absence is kept quiet and presence is not: a hundred and eighty-four voices have no oracle, and a hundred and eighty-four copies of the same word in a warning colour would bury the four that say something.
...
**A fresh clone opens on it.** There are no renders to listen to and the bank still says which voices exist and which need an oracle, neither of which requires anything to have been rendered. Otherwise the view is whichever one the last session was left on — except behind an address, which names a render and is therefore a request to listen to it.
