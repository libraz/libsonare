...
/// Bridge loop coefficients the engine derives from the patch. Mirrors
/// bowed_string_voice.cpp:141 and :145; control hash (a) moves if either does.
double patch_lp_alpha(const BowedStringPatchParams& p) {
  return 1.0 - 0.7 * (1.0 - static_cast<double>(p.brightness));
}
double patch_loss_gain(const BowedStringPatchParams& p) {
  return 0.99 - 0.09 * static_cast<double>(p.damping);
}
...
TEST_CASE("bowed probe measures the bridge force the output stands in for",
  ...
  const double violin_g = patch_loss_gain(params);
  const double violin_a = patch_lp_alpha(params);
