...
TEST_CASE("GS address table: every row decodes", "[midi][gs][address]") {
  ...
  // The extension's insertion units (40 3u xx), where the unit number is the
  // address nibble: 40 30 xx is unit 0 and reaches the storage 40 03 xx writes,
  // so the part comes back as the unit and the index is the same as the spec
  // block's. The tail past the sends is the spec block's own.
  check_row(0x403000, 0x01, GsParam::kEfxType, 0, 0);
  check_row(0x403101, 0x10, GsParam::kEfxType, 1, 1);
  check_row(0x403F02, 0x00, GsParam::kUndefined);
  check_row(0x403203, 0x40, GsParam::kEfxParameter, 0, 2);
  check_row(0x403F16, 0x40, GsParam::kEfxParameter, 19, 0x0F);
  check_row(0x403117, 0x28, GsParam::kEfxSendToReverb, 0, 1);
  check_row(0x403118, 0x00, GsParam::kEfxSendToChorus, 0, 1);
  check_row(0x403119, 0x00, GsParam::kEfxSendToDelay, 0, 1);
  check_row(0x40311A, 0x00, GsParam::kUndefined);
  check_row(0x40317F, 0x00, GsParam::kUndefined);

  // Part parameters (40 1x / 40 4x): the block nibble resolves to a channel, so
  // block 0 comes back as part 10.
  // TONE NUMBER's two bytes are one row, the index telling bank from program.
  check_row(0x401100, 0x18, GsParam::kPartToneNumber, 0, 0);
  check_row(0x401101, 0x38, GsParam::kPartToneNumber, 1, 0);
  ...
  // what tells the two assignable sources apart.
  check_row(0x40111F, 0x10, GsParam::kPartCtrlSourceNumber, 0, 0);
  check_row(0x401220, 0x11, GsParam::kPartCtrlSourceNumber, 1, 1);
  ...
  check_row(0x402240, 0x40, GsParam::kPartCtrlPitch, 0, 1);
  check_row(0x402341, 0x40, GsParam::kPartCtrlTvfCutoff, 0, 2);
  check_row(0x402442, 0x40, GsParam::kPartCtrlAmplitude, 0, 3);
  check_row(0x402543, 0x40, GsParam::kPartCtrlLfo1Rate, 0, 4);
  check_row(0x402644, 0x00, GsParam::kPartCtrlLfo1PitchDepth, 0, 5);
  check_row(0x402745, 0x00, GsParam::kPartCtrlLfo1TvfDepth, 0, 6);
  check_row(0x402846, 0x00, GsParam::kPartCtrlLfo1TvaDepth, 0, 7);
  check_row(0x402947, 0x40, GsParam::kPartCc1Dest, 0, 8);
  check_row(0x402A4A, 0x00, GsParam::kPartCc1Dest, 2, 10);
  check_row(0x402B50, 0x40, GsParam::kPartCtrlPitch, 0, 11);
  check_row(0x402C51, 0x40, GsParam::kPartCtrlTvfCutoff, 0, 12);
  check_row(0x402D52, 0x40, GsParam::kPartCtrlAmplitude, 0, 13);
  check_row(0x402E53, 0x40, GsParam::kPartCtrlLfo1Rate, 0, 14);
  check_row(0x402F54, 0x00, GsParam::kPartCtrlLfo1PitchDepth, 0, 15);
  check_row(0x402055, 0x00, GsParam::kPartCtrlLfo1TvfDepth, 0, 9);
  check_row(0x402156, 0x00, GsParam::kPartCtrlLfo1TvaDepth, 0, 0);
  check_row(0x402257, 0x40, GsParam::kPartCc2Dest, 0, 1);
  check_row(0x402358, 0x00, GsParam::kPartCc2Dest, 0, 2);
  check_row(0x40245A, 0x00, GsParam::kPartCc2Dest, 2, 3);
  check_row(0x402548, 0x00, GsParam::kPartCc1Dest, 0, 4);
  ...
  check_row(0x410005, 0x41, GsParam::kDrumMapName, 0x05, 0);
  check_row(0x411000, 0x20, GsParam::kDrumMapName, 0x00, 1);
...
const std::vector<ParseCase>& parse_value_cases() {
  static const std::vector<ParseCase> cases = {
      ...
      {{0x41, 0x10, 0x42, 0x12, 0x40, 0x41, 0x22, 0x10, 0x4D},
       GsSysExKind::kEfxPartSwitch,
       0,
       0x10},
      ...
      {{0xF0, 0x41, 0x10, 0x42, 0x12, 0x40, 0x41, 0x22, 0x03, 0x5A, 0xF7},
       ...
       0,
       3},
      // 7F is outside the row's 00-10, so it is ignored rather than read as a
      // unit — the rule every address but USE FOR RHYTHM PART follows.
      {{0x41, 0x10, 0x42, 0x12, 0x40, 0x40, 0x22, 0x7F, 0x5F}, GsSysExKind::kNone, 0, 0},
      ...
      {{0xF0, 0x41, 0x10, 0x42, 0x12, 0x00, 0x00, 0x7F, 0x7F, 0x02, 0xF7},
       ...
       0,
       0},
      {{0x41, 0x10, 0x42, 0x12, 0x40, 0x40, 0x22, 0x02, 0x5C}, GsSysExKind::kEfxPartSwitch, 9, 2},
      ...
      {{0xF0, 0x41, 0x10, 0x42, 0x12, 0x40, 0x12, 0x15, 0x11, 0x08, 0xF7},
       ...
       1,
       1},
      {{0x41, 0x10, 0x42, 0x12, 0x40, 0x41, 0x22, 0x02, 0x5B}, GsSysExKind::kEfxPartSwitch, 0, 2},
