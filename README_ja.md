...
- **解析** — BPM、キー、コード（HMM 平滑化・転回形・キーコンテキスト）、
  ...
  インパルス応答からの RT60／EDT。C50／C80／D50 は実測インパルス応答が必要で、直接音の到達
  時刻が既知でないブラインド録音では明瞭度指標を求められないため NaN になる）。librosa と
  重なる範囲ではデフォルト値を揃え、CI で librosa のリファレンス値と照合しているため、結果
  をそのまま移行できます。
- **マスタリング** — 89 個の個別の名前付き DSP プロセッサ（EQ、ダイナミクス、マルチバンド、
  ステレオ、サチュレーション、リペア、マキシマイザー、リファレンスマッチング）。
  `BUILD_FX=OFF` ではクリエイティブ系ストリーミングエフェクトが外れて 72 個になります。
...
emsdk 5.0.2 では 0.96 MiB raw / 365 KiB gzip（full entry は 4.96 MiB raw / 1.65 MiB gzip）です。
