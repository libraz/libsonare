...
**C++・Python・ブラウザのためのオーディオDSPツールキット — 解析、マスタリング、ミキシング、編集。**
...
- **編集DSP（DAW DSP primitives）** — タイムストレッチ／ピッチシフト、ピッチ補正、
  ノート区間ストレッチ、ボイスチェンジ（ピッチ＋フォルマント）、リアルタイムミキサー基本機能、
  ルーティンググラフ、ディレイ／モジュレーション／リバーブ系FX、ダッキング。
...
|----------------------------|-------------------------------|-------------------------------|
| 入力trim / fader / 極性    | センドとFXバス                | Peak / RMS / true peak        |
| Balance / stereo / dual pan| Bus insert と graph PDC       | 相関 / mono width             |
| 幅とゲインオートメーション | C / Node / Python / WASM / CLI| Golden hash とRTテスト        |
| Insert hosting / sidechain | 永続シーンミキサー            | process中のno-allocation検証  |
