...
- **UI や DAW アプリケーションワークフロー** — エディタもデバイス設定も含みません。
  libsonare はヘッドレスなアレンジメント/ランタイムコアで、その層は呼び出し側が提供します
- **プラグインホスティング**（VST/AU/CLAP の読み込み）— エンジンは自前のインサートと
  インストゥルメントをホストします。サードパーティのプラグイン形式はスコープ外です
- **リアルタイム I/O 抽象化**（PortAudio/JACK/CoreAudio ラッパー）—
  エンジンはブロックを処理し、オーディオコールバックとデバイスはホストが所有します
