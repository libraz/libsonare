...
void FdnReverb::prepare(double sample_rate, int max_block_size) {
  ...
  for (size_t i = 0; i < delays_.size(); ++i) {
    const float d = static_cast<float>(lengths[i]);
    const float g_lf = std::pow(10.0f, -3.0f * d / (t60_lf * static_cast<float>(sr)));
    const float g_hf = std::pow(10.0f, -3.0f * d / (t60_hf * static_cast<float>(sr)));
