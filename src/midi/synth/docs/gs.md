...
**The controller-destination block is `ACCEPT` in full, because it is a matrix and there is no matrix.** `40 2x xx` gives each part six controller sources — modulation, bend, channel and polyphonic aftertouch, and two assignable — each routed to the same eleven destinations: pitch, TVF cutoff, amplitude, and rate plus pitch, TVF and TVA depth for each of two LFOs. libsonare routes its controllers directly to the quantities they move, so no byte in the block has a reader, and rows are split where the accepted range or the power-on default changes rather than one per destination. Two of them name a quantity that already existed under another address, and raising one meant making it the one storage location for that quantity rather than editing the row. Both are `AUDIBLE`. `40 2x 10` BEND PITCH CONTROL is the part's bend range, sharing RPN `00 00`'s storage: it carries whole semitones above `40`, so the cents that RPN's LSB also writes have no address of their own. `40 2x 04` MODULATION LFO1 PITCH DEPTH is how much vibrato CC1 reaches at full, over the manual's `00`–`7F` → 0–600 cents; it was a constant 50 cents before it had an address, so the power-on `0A` moves a fully-raised modulation wheel to 47.2 cents. GM2's Modulation Depth Range RPN `00 05` is a second direction onto this same storage and is not decoded yet — its own default is half a semitone, which is where that 50 came from.
...
**The drum setup parameters have no power-on value, and that is what fixes their defaults.** The manual gives them none because a drum set change re-initialises them to what the kit itself specifies. So each is held at the value that changes nothing — `7F` for LEVEL and for the three multiplicand sends, `40` for PANPOT — and read only once the parameter has actually been written. A default that was not the identity would make an unwritten parameter overrule the kit. PLAY NOTE NUMBER has no such value at all, its identity being the struck note rather than a constant, so nothing but the written flag distinguishes it from an unwritten one, and the row's `def` byte is a placeholder that says nothing.

**`41 m7 rr` RX NOTE OFF is `ACCEPT`, and it is the one drum-setup parameter whose two banks disagree with nothing written.** A sampled kit piece releases when its note-off arrives; a modelled one is a one-shot, and no note-off has ever reached it. So the unwritten state already means two things, and a written `01` would mean "keep doing what you do" on one bank and "start responding" on the other — which is exactly the difference the ASSIGN MODE bullet above forbids. Settling it means choosing what a rhythm part does with a note-off in general, on both banks, and that decision is larger than the address: it moves every existing render whose drum track sends one. The row comes off `ACCEPT` in the same change that makes it. `41 m8 rr` RX NOTE ON has no such problem and is `AUDIBLE`: a note switched off is not sounded at all, on either bank, and the refusal precedes the choice of bank so a note the map has silenced cannot reach the model floor by another route.

**`41 m3 rr` ASSIGN GROUP replaces the kit piece's own exclusive class rather than adding to it, which is what makes `00` the value that says something.** The three sends beside it are multiplicands and `7F` is their identity; a group is a name, so writing one overwrites and writing `00` takes the note out of every group. That is the reachability the parameter exists for — an open hi-hat that should ring through the closed one, a kit whose two whistles were put in one group and should not be. Both banks read the same value: the SoundFont zone's `exclusiveClass` and the model kit piece's group are the same quantity from two directions, and the model voice holds the group it was STARTED in rather than reading its patch, because one kit piece is shared by every note that resolves to it. A group is also the one drum parameter that says nothing about a single note: it is a relation, so a probe has to put a second note in the group before the first can be choked by anything.

**PLAY NOTE NUMBER substitutes the sound and not the key.** The written note selects the SoundFont zone and resolves the voice parameters, and on the model floor it picks the kit piece and voices it; the struck note keeps the per-note slab entry the edits live in, keeps the exclusive-group choke, and keeps the voice, because that is what a note-off matches. The two banks agree on all of it, which is what makes a remapped kit sound the same whichever one answered. Substituting only the kit piece and leaving the voiced note behind still moves the render — so the audibility gate passes it — and no longer sounds like the note it names; that is a per-parameter identity assertion's to catch, not a did-anything-move probe's.
...
**`40 1x 1D`/`1E` KEY RANGE is a refusal and not a mute.** A key outside the part's range is not received: it takes no voice, it does not choke what the part is already sounding, and it does not spend an armed portamento — so the test sits ahead of all three in `note_on`, which is also what puts it ahead of the choice of voice bank. A low above the high names no key at all, which is what the two bytes say rather than something to be corrected into a range. The audible half is a silent render; the rest needs a note already playing to be taken away from, and each half is checked against the same strike with the range open so that "nothing happened" cannot pass on a part that was inert anyway.

## A user drum set is a kit the file builds, and each of its notes names where its sound comes from

`21 dn rr` is two drum sets a file assembles note by note, `d` selecting the set, `n` the parameter and `rr` the note. A rhythm part selects one with **program 64 or 65** — numbers `kGsDrumKits` leaves free, which is what lets them mean this. Nibble `0` is the set's twelve-character name and is `ACCEPT`, as `41 m0 rr` is. Nibbles `1`–`9` repeat the drum setup block's parameters, level for level and send for send, and carry its levels and defaults with them: `21 d7 rr` RX NOTE OFF is `ACCEPT` for exactly the reason `41 m7 rr` is. `A`, `B` and `C` are the block's own and are the whole difference between a user set and a live edit: they say which preset kit each note takes its sound from, and which note within it.

**The set is the stored kit and `41 mn rr` is the edit on top of it.** A note resolves its source first — the kit from `21 dB rr`, the note within it from `21 dC rr` — and then reads one merged set of per-note edits: the set's stored ones with the map's written ones over them, field by field behind the flag that owns each. Layered rather than one re-initialising the other, which is what lets both sides hold the value that changes nothing: a set nobody wrote sounds exactly what the part sounded before it was selected, so selecting one is audible only through what has been written into it. Both voice banks read the merged struct rather than each consulting the two slabs, so a note cannot be edited differently depending on which answered it.

**`21 dC rr` SOURCE NOTE NUMBER moves the voiced note, not the key**, on the same terms as PLAY NOTE NUMBER: the source note picks the SoundFont zone and the model floor's kit piece *and* is the note the voice is started at, while the struck note keeps the per-note slab, the choke and the voice, because that is what a note-off matches. Leaving the voiced note on the struck one gives the right kit piece at the wrong pitch, which renders plausibly and is not the note that was asked for — an identity assertion's to catch, not a did-anything-move probe's.

**`21 dA rr` SOURCE DRUM SET is `ACCEPT`.** It says which generation the source program is read in, and `kGsDrumKits` reaches every kit of every generation from the program alone, so a written map can only take a kit away rather than reach one. Raising it means deciding that emulating a module's narrower kit list is worth having, which is a different question from the one the other two answer.
