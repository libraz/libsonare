...
- **A drum note's reverb, chorus and delay sends are multiplicands of the part's send, not additions to it.** The manual is explicit (`0.0 – 1.0`, "Multiplicand of the part reverb level"). Multiplying gives the hardware behaviour that taking a part's send to zero silences its drum notes' sends too; adding does not. **Delay meets this and reverb and chorus do not yet** — outstanding work rather than a second decision. There is no delay generator in SoundFont, so the delay multiplicand has only the part's own send to scale and scales it. The other two scale the zone's `reverbEffectsSend` / `chorusEffectsSend` generator, and the part's CC91 / CC93 contribution is summed after that and never scaled — so a part taken to zero still sends whatever its kit's zones ask for. Closing it moves two parameters that already sound, so it wants its own change and its own golden check.
