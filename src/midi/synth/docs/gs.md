...
`21 dn rr` is two drum sets a file assembles note by note, `d` selecting the set, `n` the parameter and `rr` the note. A rhythm part selects one with **program 64 or 65** — numbers `kGsDrumKits` leaves free, which is what lets them mean this. Nibbles `1`–`9` repeat the drum setup block's parameters; `A`, `B` and `C` are the block's own and are the whole difference between a user set and a live edit: they say which preset kit each note takes its sound from, and which note within it.

**The set is the stored kit and `41 mn rr` is the edit on top of it.** A note resolves its source first — the kit from `21 dB rr`, the note within it from `21 dC rr` — and the map's own edits then apply to what that produced, PLAY NOTE NUMBER included. Both are layered rather than one re-initialising the other, for the reason the drum-setup defaults paragraph gives: every parameter on both sides is held at the value that changes nothing, so a set nobody wrote sounds exactly what the part sounded before it was selected, and selecting one is audible only through what has been written into it.
