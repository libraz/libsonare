...
**The two references cannot both be reached, and that is measured rather than argued.** A fit of the brightness family against the direct reference takes the voice from 26.63 to 3.42 dB RMS of it above 2 kHz and generalises to held-out notes — and takes program 29's whole chain from 4.93 to 23.29 dB of the amplified reference. **No rig recovers that**: a cabinet only darkens, and the fitted chain would need 27 dB more at 8 kHz than it has. Neither can the amplified reference be reached from a real direct signal through a real cabinet — that lands some 30 dB under it — so the module's sample is not a plausible amplifier on a plausible pickup either. Two products, thirty decibels apart, and neither is ground truth: what the direct reference can do here is bound the voice, not target it, and the fitted values are recorded in `tools/voicematch/calibrations.json` rather than adopted.

So the bank keeps a voice that is not an electric guitar at the pickup and a chain that lands where a module does — this page's own rule, broken, and left broken because the fix above costs more than it buys. It is recorded here so that what the next person finds is a measurement rather than an oversight. What would settle it is a second reference at the pickup: two products there would say whether this one's darkness is the instrument or the library, which is the one question a single direct capture cannot answer however well it was made.
