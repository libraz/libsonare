...
**What that does not settle is what to fit.** The two references cannot be chained: put a cabinet on this direct signal and the result lands some 30 dB under the amplified reference at 8 kHz, so the module sampled a far brighter instrument than the one captured here. Fitting the patch wholesale to one dark guitar would voice five programs after it and send the whole chain below any module, so the direct reference bounds the voice before it targets it.
