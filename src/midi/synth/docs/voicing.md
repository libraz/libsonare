...
**There is one amplifier.** `src/mastering/saturation/` already holds a circuit-level one with a generated cabinet impulse and calibrated rigs in physical units. A second implementation inside the synth would mean two calibrated amplifiers to keep in agreement, which is the failure mode the cabinet generator's own history warns about. The same rule retires a room mic: a rig reaches a room through `effects.reverb.room`, which synthesises its response from geometry, rather than through a second reverberator inside the amplifier. What that costs is the blend point — the room hears the close microphone's colouration rather than the cabinet's — and that difference is not worth a duplicate.

**A rig is a chain, not one insert.** A pedal ahead of the amplifier and a rack stage behind it are stages beside it rather than a different mechanism, and the vocabulary is the GS multi-effect's own (`gs_efx_insert_chain` builds GTR Multi out of the same blocks), so a bank rig and a file's own rig are made of one set of parts. A host supplies one in the mixing scene's insert vocabulary — `processor` and `params`, in signal order — so a host that can describe a channel strip describes a rig with the same words and needs no second format. The synth takes the decoded list rather than the JSON, which is what keeps it from linking the mixing module to read one.

**Three gains, and they are not interchangeable.** The input trim is what reaches the amplifier: a voicing's drive curve is calibrated for a full-scale signal and the bank's guitar arrives well under one, so without it a driven binding reads a part of the curve that is still clean — which is exactly what program 29 did, its crest rising above the direct signal's where an amplifier is supposed to compress it. The output trim sits after the amplifier and is loudness only, measured identical crest from 0 to 60 dB, so it peaks a program against its unrigged siblings and nothing else. Drive moves the triode's operating point *and* the bright-cap shelf in front of it, so buying saturation with it also buys brightness; a driven binding buys its saturation with the trim and keeps drive for the character.

**A tone control at rest is absent, not flat.** A binding carries the amplifier's front panel — bass, mid, treble, presence — and the amp builder reads an absent key as "keep the preset's own voicing". A preset is a voiced amplifier rather than a flat one, so a control that has not been turned must be left out of the params entirely; sending it as 0 dB would flatten a stack the preset voiced on purpose, and it would do it silently. What this cannot express is exactly flat on an amplifier voiced away from flat.
...
**Where the three measured guitars stand, and what a binding can still do for them.** 29's chain already falls at the reference's own rate from the reference's own knee (−40.2 dB/oct from 4 kHz against −40.5 from 4 kHz) and what is left is two-sided rather than a tilt: 9 dB hot at 6.35 kHz and 20 short at 12.7. Drive does not reach it on that reading — across 0.45 to 0.95 the error from 2 to 10 kHz stays inside 0.8 dB, and every dB the wider band appears to gain comes from the top two thirds alone. The centroid disagrees over the same knob and the same range, moving brightness 43.60 to 31.26 per cent of the reference, so one of the two is measuring a redistribution the other cannot see; which is open, and the fitted result below rests on the centroid. 30 runs the same amplifier at its own gain and carries the same residual, so the two are one shape measured twice; with the rig off they render identically, which is this section's claim that they are one instrument rather than two, measured. 31 is the one whose chain is short of its reference — 13 dB at 4 kHz — and no drive the clean binding accepts closes more than 5 of it, so what that program needs is not a different binding.

**A direct capture arrived, and it says the rig has been carrying the voice.** Every guitar reference here was recorded through an amplifier until now, so an excess belonged to the chain and neither half could be held to it; `electric_guitar_di` is on the other side of the boundary. Against it the model's bare pickup is 12 dB hot at 2 kHz rising to 40 at 6.35, and falls at 17.9 dB/oct where the reference falls at 23.7 — so at 8 kHz the bound rig removes 18 dB where 41 was wanted, and every drive and trim in the table was fitted to a chain whose voice was already wrong. That does not make them wrong, but it does make them unearned. The electric bass is the control and it clears the string model rather than the guitar: measured the same way against its own direct reference the slopes agree within 2 dB/oct and the model is the darker of the two, so this is `electric_guitar`'s brightness and not Karplus-Strong's.

**The direct fit does not cost the amplified end, and a claim that it did came from an override that never arrived.** A patch prefix reaches that patch alone, and `overdriven` and `distortion` are copies taken from `electric_guitar`'s compiled-in values before each is tuned under its own name — so `electric_guitar.…=` moves programs 26 and 27 and leaves 28, 29 and 30 rendering byte-identical to the unmodified build, where a source edit would have reached all five. Carried onto `overdriven` instead and measured against that program's own reference over all 35 notes, the fit improves the chain at the shipped rig gain rather than breaking it: brightness 43.60 to 25.27 per cent of the reference centroid, the partial stack 7.28 to 4.45 dB, body under the note 46.69 to 35.27, the double decay 10.43 to 5.56 dB/s, and the five rows whose two sides covered different parts of the decay all become comparable. Driving the crunch rig harder centres brightness — 22.29 per cent at 0.75, its best — and sells scatter for it, the stack going back out to 4.93 dB and tone-to-noise to 8.78. **Drive reaches this band**: on the unfitted voice alone it moves brightness 43.60 to 31.26 across 0.55 to 0.95.

So the fit is a candidate for a reason that has nothing to do with the amplifier. One product at the pickup can bound a voice and cannot target it — nothing here says whether `electric_guitar_di`'s darkness is the instrument or the library — and that is the one question a single direct capture cannot answer however well it was made. What would settle it is a second reference at the pickup. Until then the values stay in `tools/voicematch/calibrations.json`, under both the program they were fitted on and the amplified program they were checked against.
...
The gains, the tone stack and the two slot selectors are ordinary tunables and appear as keys. The **preset name** is not one: a float selects it by index, but an index says nothing about which amplifier it names, so reordering or renaming the table would move no fingerprint while changing every rig. That is what the `#rig<TAB>NNN<TAB>preset` line in the dump is for — it reports the name the library resolved, and it is folded into the table's unit as `rig.NNN`.
