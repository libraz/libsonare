...
class ReedVoiceCore {
 public:
  ...
  /// bounded to the reed's stable oscillating band so more breath colours the
 ...
 private:
  ...
  // (feedback register + loop-filter phase). The cylinder uses half the
  // period (negative-feedback comb, odd harmonics); the cone the full period.
  // prefill_span is how much of the line the onset seeds — the note's own
  // loop period plus the margin the seed needs, which is shorter than the
  // line itself. out is the last delay-line output (the pressure returning to
  // the reed next sample).
  ...
  // start at reed's own 0.6/0.5 (ExcitationBases's in-class defaults are
  // placeholders shared with other engines, not this one's).
  ExcitationBases excite_{0.6f, 0.5f, 0.0f, 0.0f};
