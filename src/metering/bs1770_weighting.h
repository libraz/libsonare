...
inline double bs1770_channel_weight(int channel, int channels) noexcept {
  switch (channels) {
    ...
    case 6:
      ...
      return 1.0;
    case 8:
      // 7.1: L, R, C, LFE(3), Ls(4), Rs(5), Lss(6), Rss(7) (canonical order, see
      // core/channel_layout.h). NON-NORMATIVE: BS.1770-4 does not define a 7.1
      // weighting; the side pair (Lss/Rss) is treated like the rear/back surrounds
      // (Ls/Rs, +1.5 dB) as a reasonable extension rather than a standard value.
      if (channel == 3) return 0.0;                    // LFE excluded.
