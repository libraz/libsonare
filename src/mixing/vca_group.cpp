...
// own gain (in dB) additively to the member's accumulated VCA-group offset;
// summing in the dB domain is equivalent to multiplying the linear VCA gains,
// which is the intended cascaded-fader behaviour. Every group applies *deltas*
// via add_vca_group_offset_db() so it only ever touches its own contribution
// and never disturbs the strip's manual trim (set_vca_offset_db) or other
...
bool VcaGroup::add_member(GainProcessor* gain) {
  ...
  gain->add_vca_group_offset_db(vca_gain_db_);
  return true;
}
...
bool VcaGroup::remove_member(GainProcessor* gain) {
  ...
  (*found)->add_vca_group_offset_db(-vca_gain_db_);
...
void VcaGroup::set_vca_gain_db(float gain_db) noexcept {
  ...
  for (GainProcessor* member : members_) {
    member->add_vca_group_offset_db(delta);
  }
}
