...
std::vector<float> pad_center(const float* x, std::size_t n, std::size_t size, float pad_value) {
  if (size < n) {
    throw std::invalid_argument("pad_center: target size smaller than input");
  }
  if (n > 0 && x == nullptr) {
    throw std::invalid_argument("pad_center: null input with non-zero length");
  }
...
std::vector<float> fix_length(const float* x, std::size_t n, std::size_t size, float pad_value) {
  if (n > 0 && x == nullptr) {
    throw std::invalid_argument("fix_length: null input with non-zero length");
  }
  if (n == size) {
...
std::vector<int> fix_frames(const std::vector<int>& frames, int x_min, int x_max, bool pad) {
  ...
  for (std::size_t i = 1; i < frames.size(); ++i) {
    if (frames[i] < frames[i - 1]) {
      throw std::invalid_argument("fix_frames: frames must be non-decreasing");
    }
  }
  if (x_min >= 0) {
    for (int f : frames) {
      if (f < 0) {
        throw std::invalid_argument("fix_frames: negative frame with x_min >= 0");
      }
    }
  }
