...
class NoiseTracker {
 public:
  ...
  /// @brief Binds the tracker to @p mode, which is named rather than defaulted:
  ///        no mode is right for every programme, and the one a default would
  ///        have to pick is the one the note above tells callers to avoid.
  NoiseTracker(int n_bins, int sample_rate, Mode mode, int hop_length = 512);
