...
/// Crossover dead zone at `crossover == 1` — a half-width in the power stage's
/// normalized domain — and how much sharper each half's turn-on gets with it.
/// The half-width is measured against the stage's own working range, so a driven
/// signal traverses the zone every cycle while a quiet one sits inside it.
///
/// The coupling between them is load-bearing. The composite's origin slope is
/// knee*sech^2(knee*bias), where knee enters unsquared and bias squared, so
/// driving knee off 1 at the same rate as bias makes the slope RISE before it
/// falls. Scaling the knee's excess with bias SQUARED cancels that: the slope
/// becomes `1 + (kCrossoverKneeSharpness-1)*bias^2`, non-increasing exactly when
/// the sharpness is <= 1, which the static_assert below enforces rather than
/// merely documenting.
///
/// That square costs depth at a given bias, so the half-width has to reach
/// further to keep `coldBiasBuzz`'s "dead zone audible" setting (crossover
/// 0.875) at the -24 dB depth it was voiced against. 1.3 is what lands it; a
/// rounder value undershoots.
...
bool AmpSim::ChannelChain::discard_non_finite() noexcept {
  const auto discard_cab = [](CabStage& stage) noexcept {
    bool any = discard_group_if_non_finite(stage.hp.z1, stage.hp.z2);
    any |= discard_group_if_non_finite(stage.bump.z1, stage.bump.z2);
    any |= discard_group_if_non_finite(stage.presence.z1, stage.presence.z2);
    any |= discard_group_if_non_finite(stage.lp1.z1, stage.lp1.z2);
    any |= discard_group_if_non_finite(stage.lp2.z1, stage.lp2.z2);
    any |= discard_group_if_non_finite(stage.mic_prox.z1, stage.mic_prox.z2);
    any |= discard_group_if_non_finite(stage.mic_presence.z1, stage.mic_presence.z2);
    any |= discard_group_if_non_finite(stage.mic_top.z1, stage.mic_top.z2);
    return any;
  };
  bool discarded = discard_group_if_non_finite(pre.z1, pre.z2);
  discarded |= discard_group_if_non_finite(bass.z1, bass.z2);
  discarded |= discard_group_if_non_finite(mid.z1, mid.z2);
  discarded |= discard_group_if_non_finite(treble.z1, treble.z2);
  discarded |= discard_group_if_non_finite(nfb_shape.z1, nfb_shape.z2);
  discarded |= discard_cab(cab_a);
  discarded |= discard_cab(cab_b);
  for (PreampStage& stage : stages) {
    ...
    discarded |= discard_group_if_non_finite(stage.hp_x1, stage.hp_y1, stage.cathode.z1,
                                             stage.cathode.z2, stage.grid_env);
  }
  discarded |= discard_group_if_non_finite(stack.s1, stack.s2, stack.s3);
...
void AmpSim::process(float* const* channels, int num_channels, int num_samples) {
  ...
  for (int ch = 0; ch < num_channels; ++ch) {
    discarded |= chains_[static_cast<size_t>(ch)].discard_non_finite();
  }
