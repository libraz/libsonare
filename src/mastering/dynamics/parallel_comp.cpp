...
void ParallelComp::process(float* const* channels, int num_channels, int num_samples) {
  ...
  const float ceiling = db_to_linear(config_.output_ceiling_db);
  if (config_.linked_detection) {
    for (int i = 0; i < num_samples; ++i) {
      ...
      for (int ch = 0; ch < num_channels; ++ch) {
        ...
        float out = dry * (1.0f - config_.mix) + compressed * config_.mix;
        if (config_.output_limiter && std::abs(out) > ceiling) out = std::copysign(ceiling, out);
        channels[ch][i] = out;
      }
