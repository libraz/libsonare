"""libsonare - audio analysis library (Python binding)."""

from importlib.metadata import PackageNotFoundError
from importlib.metadata import version as _pkg_version

from ._effects_polyphony import PolyphonicAnalysis
from ._project import (
    AssistSidecar,
    BuiltinSynthConfig,
    ExternalInstrument,
    MidiCcBinding,
    MidiRouteResult,
    NotePairValidation,
    Project,
    ProjectDeserializeResult,
    SampleBank,
    Sf2InstrumentConfig,
    Sf2ProgramStatus,
    SynthModRouting,
    SynthPatch,
    TakeAlignment,
    align_take_to_reference,
    controller_profile_names,
    project_abi_version,
    synth_enum_tables,
    synth_gs_drum_kit_is_voiced_apart,
    synth_gs_drum_kit_name,
    synth_gs_variation_is_voiced_apart,
    synth_preset_names,
    synth_preset_patch,
)
from ._runtime import ErrorCode, SonareError, SonareValueError
from .analyzer import (
    Mixer,
    MixerStereoResult,
    NoteEdit,
    NoteObject,
    NoteTarget,
    PercussiveEvent,
    PercussiveEventEdit,
    PitchDecomposition,
    RealtimeVoiceChanger,
    RealtimeVoiceChangerConfig,
    SilenceCommonReport,
    SpectralRegionOp,
    StreamingEqualizer,
    StreamingMasteringChain,
    StreamingRetune,
    TranscribeResult,
    abi_version,
    agglomerative,
    amplitude_to_db,
    analyze,
    analyze_bpm,
    analyze_dynamics,
    analyze_impulse_response,
    analyze_melody,
    analyze_rhythm,
    analyze_sections,
    analyze_timbre,
    analyze_with_progress,
    assign_note_targets,
    bass_chroma,
    capabilities,
    capability_catalog,
    chirp,
    chord_functional_analysis,
    chroma,
    chroma_cens,
    chroma_cqt,
    clicks,
    cqt,
    cqt_to_audio,
    cross_similarity,
    cyclic_tempogram,
    db_to_amplitude,
    db_to_power,
    decompose,
    decompose_note_pitch,
    decompose_stems,
    decompose_stems_linked,
    decompose_with_init,
    deemphasis,
    detect_acoustic,
    detect_beats,
    detect_boundaries,
    detect_bpm,
    detect_chords,
    detect_downbeats,
    detect_key,
    detect_key_candidates,
    detect_onsets,
    ebur128_loudness_range,
    engine_abi_version,
    estimate_meter,
    estimate_room,
    estimate_tuning,
    extract_notes,
    extract_percussive_events,
    fix_frames,
    fix_length,
    fourier_tempogram,
    frame_signal,
    frames_to_samples,
    frames_to_time,
    griffin_lim,
    harmonic,
    has_ffmpeg_support,
    hpss,
    hpss_with_residual,
    hybrid_cqt,
    hz_to_mel,
    hz_to_midi,
    hz_to_note,
    lag_to_recurrence,
    lufs,
    lufs_interleaved,
    lufs_series_interleaved,
    master_audio,
    master_audio_stereo,
    mastering,
    mastering_ab_match_loudness,
    mastering_assistant_suggest,
    mastering_assistant_suggest_chain,
    mastering_assistant_suggest_chain_stereo,
    mastering_assistant_suggest_stereo,
    mastering_audio_profile,
    mastering_audio_profile_stereo,
    mastering_chain,
    mastering_chain_stereo,
    mastering_dynamics_compressor,
    mastering_dynamics_gate,
    mastering_dynamics_transient_shaper,
    mastering_insert_names,
    mastering_insert_param_info,
    mastering_insert_param_names,
    mastering_insert_timing,
    mastering_pair_analysis_names,
    mastering_pair_analyze,
    mastering_pair_process,
    mastering_pair_processor_names,
    mastering_platform_names,
    mastering_preset_names,
    mastering_preset_params,
    mastering_process,
    mastering_process_stereo,
    mastering_processor_catalog,
    mastering_processor_names,
    mastering_repair_declick,
    mastering_repair_declick_stereo,
    mastering_repair_declip,
    mastering_repair_declip_stereo,
    mastering_repair_decrackle,
    mastering_repair_decrackle_stereo,
    mastering_repair_dehum,
    mastering_repair_dehum_stereo,
    mastering_repair_denoise_classical,
    mastering_repair_denoise_classical_linked,
    mastering_repair_denoise_classical_stereo,
    mastering_repair_dereverb_classical,
    mastering_repair_dereverb_classical_linked,
    mastering_repair_dereverb_classical_stereo,
    mastering_repair_dereverb_config_for_room,
    mastering_repair_detect_clicks,
    mastering_repair_detect_clipping,
    mastering_repair_detect_crackle,
    mastering_repair_detect_hum,
    mastering_repair_detect_noise_floor,
    mastering_repair_detect_reverb,
    mastering_repair_detect_trim_range,
    mastering_repair_detect_trim_range_stereo,
    mastering_repair_noise_band_bins,
    mastering_repair_trim_silence,
    mastering_repair_trim_silence_stereo,
    mastering_stereo_analysis_names,
    mastering_stereo_analyze,
    mastering_streaming_preview,
    mastering_streaming_preview_stereo,
    mel_delta,
    mel_spectrogram,
    mel_to_audio,
    mel_to_hz,
    mel_to_stft,
    merge_notes,
    metering_crest_factor_db,
    metering_crest_factor_db_stereo,
    metering_dc_offset,
    metering_detect_clipping,
    metering_dynamic_range,
    metering_peak_db,
    metering_phase_scope,
    metering_phase_scope_decimated,
    metering_rms_db,
    metering_silence_ratio,
    metering_spectrum,
    metering_spectrum_frame,
    metering_stereo_correlation,
    metering_stereo_width,
    metering_true_peak_db,
    metering_vectorscope,
    metering_vectorscope_decimated,
    mfcc,
    mfcc_to_audio,
    mfcc_to_mel,
    midi_to_hz,
    mix_stereo,
    mixing_scene_preset_json,
    mixing_scene_preset_names,
    momentary_lufs,
    nn_filter,
    nnls_chroma,
    normalize,
    normalize_rms,
    normalize_rms_stereo,
    normalize_stereo,
    note_move,
    note_segments,
    note_stretch,
    note_targets_from_smf,
    note_to_hz,
    onset_backtrack,
    onset_envelope,
    onset_strength_multi,
    pad_center,
    path_enhance,
    pcen,
    peak_pick,
    percussive,
    phase_vocoder,
    piptrack,
    pitch_correct_timevarying,
    pitch_correct_to_midi,
    pitch_correct_to_midi_timevarying,
    pitch_pyin,
    pitch_shift,
    pitch_tuning,
    pitch_yin,
    plp,
    poly_features,
    power_to_db,
    preemphasis,
    pseudo_cqt,
    realtime_voice_changer_preset_config,
    realtime_voice_changer_preset_json,
    realtime_voice_changer_preset_names,
    realtime_voice_changer_preset_pod,
    reassigned_spectrogram,
    recurrence_matrix,
    recurrence_to_lag,
    remix,
    remix_aligned_intervals,
    render_notes,
    render_percussive_events,
    resample,
    rms_energy,
    room_morph,
    samples_to_frames,
    scale_correction_semitones,
    scale_pitch_class_enabled,
    scale_quantize_midi,
    short_term_lufs,
    spectral_bandwidth,
    spectral_centroid,
    spectral_contrast,
    spectral_edit,
    spectral_flatness,
    spectral_flux,
    spectral_rolloff,
    split_note,
    split_silence,
    split_silence_common,
    split_silence_common_with_report,
    stft,
    stft_db,
    subsegment,
    synthesize_rir,
    tempogram,
    tempogram_ratio,
    time_stretch,
    time_to_frames,
    tone,
    tonnetz,
    transcribe,
    trim,
    trim_silence,
    validate_realtime_voice_changer_preset_json,
    vector_normalize,
    version,
    voice_change,
    voice_change_realtime,
    voice_changer_abi_version,
    voice_character_preset_id,
    vqt,
    vqt_to_audio,
    waveform_peak_pyramid,
    waveform_peaks,
    zero_crossing_rate,
    zero_crossings,
)
from .audio import Audio
from .engine import ClipPageProvider, FileClipPageProvider, RealtimeEngine
from .mixing_assistant import (
    MixTrackInput,
    mix_source_class_from_name,
    mix_source_class_names,
    suggest_mix_scene,
    suggest_mix_scene_json,
)
from .streaming import StreamAnalyzer
from .types import (
    AcousticResult,
    AnalysisBeatObservations,
    AnalysisDynamics,
    AnalysisMelody,
    AnalysisResult,
    AnalysisRhythm,
    AnalysisTimbre,
    AutomationCurve,
    AutomationPoint,
    BpmAnalysisResult,
    BpmCandidate,
    BpmHypothesis,
    Capabilities,
    CapabilitiesAbi,
    CapabilitiesDecode,
    CapabilitiesFeatures,
    CapabilityCatalog,
    CapabilityCatalogPresets,
    ChannelLayout,
    Chord,
    ChordAnalysisResult,
    ChromaResult,
    ClickDetection,
    ClipDetection,
    ClipPageRequest,
    ClippingRegion,
    ClippingReport,
    CqtResult,
    CrackleDetection,
    DeclickReport,
    DeclickStereoResult,
    DeclipReport,
    DeclipStereoResult,
    DecrackleReport,
    DecrackleStereoResult,
    DehumReport,
    DehumStereoResult,
    DenoiseLinkedResult,
    DenoiseReport,
    DenoiseStereoResult,
    DereverbClassicalConfig,
    DereverbLinkedResult,
    DereverbReport,
    DereverbStereoResult,
    DynamicRangeReport,
    DynamicsResult,
    EngineBounceOptions,
    EngineBounceResult,
    EngineCaptureStatus,
    EngineClip,
    EngineFreezeOptions,
    EngineFreezeResult,
    EngineGraphConnection,
    EngineGraphMix,
    EngineGraphNode,
    EngineGraphNodeType,
    EngineGraphParameterBinding,
    EngineGraphSpec,
    EngineMarker,
    EngineMetronomeConfig,
    EngineMidiClipSchedule,
    EngineMidiEvent,
    EngineTelemetry,
    EngineTelemetryError,
    EngineTelemetryType,
    EngineTrackMonitorMode,
    EqSpectrumSnapshot,
    ExternalMidiEvent,
    GoniometerPoint,
    HpssResult,
    HumDetection,
    InverseResult,
    Key,
    KeyCandidate,
    KeyProfile,
    LoudnessMatch,
    LufsResult,
    MarkerKind,
    MasteringChainResult,
    MasteringChainStereoResult,
    MasteringChannelPolicy,
    MasteringInsertParamChoice,
    MasteringInsertParamInfo,
    MasteringInsertTiming,
    MasteringLoudnessSummary,
    MasteringPresetCatalogEntry,
    MasteringPresetKind,
    MasteringProcessorCatalogEntry,
    MasteringProcessorCategory,
    MasteringProcessorKind,
    MasteringReport,
    MasteringResult,
    MasteringStereoResult,
    MelodyPoint,
    MelodyResult,
    MelSpectrogramResult,
    MeterEstimate,
    MeterTap,
    MeterTelemetryRecord,
    MeterTelemetryRecordWide,
    MfccResult,
    MixMeterSnapshot,
    MixResult,
    Mode,
    NoiseDetection,
    NormalizeStereoResult,
    NoteSegment,
    PanLaw,
    ParameterInfo,
    PhaseScopeReport,
    PitchClass,
    PitchResult,
    ProjectClip,
    ProjectMarker,
    ProjectSource,
    ProjectTrack,
    QuantizeConfig,
    ReverbDetection,
    RhythmResult,
    RirDiagnostic,
    RirResult,
    RoomEstimate,
    RoomMorphResult,
    ScopeTelemetryRecord,
    Section,
    SectionResult,
    SectionType,
    SendTiming,
    SpectrumReport,
    StageGainReduction,
    StftResult,
    StreamBarChord,
    StreamChordChange,
    StreamConfig,
    StreamFrames,
    StreamFramesI16,
    StreamFramesU8,
    StreamPatternScore,
    StreamStats,
    TimbreFrame,
    TimbreResult,
    TimeSignature,
    TransportState,
    TrimRange,
    TrimReport,
    TrimSilenceStereoResult,
    VectorscopeReport,
    WaveformPeaksReport,
)

try:
    __version__ = _pkg_version("libsonare")
except PackageNotFoundError:
    # Source checkout / editable install without metadata: fall back to the
    # native library's version string so ``libsonare.__version__`` is always
    # populated.
    __version__ = version()

__all__ = [
    "AcousticResult",
    "AnalysisBeatObservations",
    "AnalysisDynamics",
    "AnalysisMelody",
    "AnalysisResult",
    "AnalysisRhythm",
    "AnalysisTimbre",
    "AssistSidecar",
    "Audio",
    "CapabilityCatalog",
    "CapabilityCatalogPresets",
    "Capabilities",
    "CapabilitiesAbi",
    "CapabilitiesDecode",
    "CapabilitiesFeatures",
    "ClipPageProvider",
    "FileClipPageProvider",
    "RirDiagnostic",
    "RirResult",
    "RoomMorphResult",
    "RoomEstimate",
    "estimate_room",
    "room_morph",
    "synthesize_rir",
    "AutomationPoint",
    "AutomationCurve",
    "BpmAnalysisResult",
    "BpmCandidate",
    "BpmHypothesis",
    "BuiltinSynthConfig",
    "ChannelLayout",
    "Chord",
    "ChordAnalysisResult",
    "ClickDetection",
    "ClipDetection",
    "ClipPageRequest",
    "__version__",
    "ChromaResult",
    "ClippingRegion",
    "ClippingReport",
    "CqtResult",
    "CrackleDetection",
    "DeclickReport",
    "DeclickStereoResult",
    "DeclipReport",
    "DeclipStereoResult",
    "DecrackleReport",
    "DecrackleStereoResult",
    "DehumReport",
    "DehumStereoResult",
    "DenoiseLinkedResult",
    "DenoiseReport",
    "DenoiseStereoResult",
    "DereverbClassicalConfig",
    "DereverbLinkedResult",
    "DereverbReport",
    "DereverbStereoResult",
    "DynamicRangeReport",
    "DynamicsResult",
    "EngineBounceOptions",
    "EngineBounceResult",
    "EqSpectrumSnapshot",
    "EngineCaptureStatus",
    "EngineClip",
    "EngineFreezeOptions",
    "EngineFreezeResult",
    "EngineTelemetry",
    "EngineTelemetryError",
    "EngineTelemetryType",
    "EngineTrackMonitorMode",
    "EngineGraphConnection",
    "EngineGraphMix",
    "EngineGraphNode",
    "EngineGraphNodeType",
    "EngineGraphParameterBinding",
    "EngineGraphSpec",
    "EngineMarker",
    "EngineMetronomeConfig",
    "EngineMidiClipSchedule",
    "EngineMidiEvent",
    "ExternalInstrument",
    "ExternalMidiEvent",
    "GoniometerPoint",
    "HpssResult",
    "HumDetection",
    "InverseResult",
    "Key",
    "KeyCandidate",
    "KeyProfile",
    "LoudnessMatch",
    "LufsResult",
    "MasteringChainResult",
    "MasteringChainStereoResult",
    "MasteringLoudnessSummary",
    "MasteringChannelPolicy",
    "MasteringInsertParamChoice",
    "MasteringInsertParamInfo",
    "MasteringInsertTiming",
    "MasteringPresetCatalogEntry",
    "MasteringPresetKind",
    "MasteringProcessorCatalogEntry",
    "MasteringProcessorCategory",
    "MasteringProcessorKind",
    "MasteringReport",
    "MasteringResult",
    "MasteringStereoResult",
    "MarkerKind",
    "MelodyPoint",
    "MelodyResult",
    "MelSpectrogramResult",
    "MeterEstimate",
    "MeterTap",
    "MeterTelemetryRecord",
    "MeterTelemetryRecordWide",
    "MidiCcBinding",
    "MidiRouteResult",
    "MixMeterSnapshot",
    "MixResult",
    "MixTrackInput",
    "MfccResult",
    "Mode",
    "NoiseDetection",
    "NormalizeStereoResult",
    "NoteSegment",
    "NotePairValidation",
    "PanLaw",
    "ParameterInfo",
    "PhaseScopeReport",
    "PitchClass",
    "PitchResult",
    "Project",
    "ProjectDeserializeResult",
    "ProjectClip",
    "ProjectMarker",
    "ProjectSource",
    "ProjectTrack",
    "QuantizeConfig",
    "RealtimeEngine",
    "ReverbDetection",
    "RhythmResult",
    "ScopeTelemetryRecord",
    "SampleBank",
    "Section",
    "SectionResult",
    "SectionType",
    "SendTiming",
    "Sf2InstrumentConfig",
    "Sf2ProgramStatus",
    "SilenceCommonReport",
    "SynthModRouting",
    "SynthPatch",
    "SonareError",
    "SonareValueError",
    "ErrorCode",
    "SpectrumReport",
    "StageGainReduction",
    "StftResult",
    "StreamAnalyzer",
    "StreamBarChord",
    "StreamChordChange",
    "StreamConfig",
    "StreamFrames",
    "StreamFramesI16",
    "StreamFramesU8",
    "StreamPatternScore",
    "StreamStats",
    "StreamingMasteringChain",
    "StreamingEqualizer",
    "TakeAlignment",
    "TimbreFrame",
    "TimbreResult",
    "TimeSignature",
    "TransportState",
    "TrimRange",
    "TrimReport",
    "TrimSilenceStereoResult",
    "VectorscopeReport",
    "WaveformPeaksReport",
    "abi_version",
    "align_take_to_reference",
    "analyze",
    "analyze_bpm",
    "analyze_dynamics",
    "analyze_impulse_response",
    "analyze_melody",
    "analyze_rhythm",
    "analyze_sections",
    "analyze_timbre",
    "analyze_with_progress",
    "amplitude_to_db",
    "bass_chroma",
    "capabilities",
    "chord_functional_analysis",
    "chroma",
    "chirp",
    "clicks",
    "chroma_cens",
    "chroma_cqt",
    "cqt",
    "tone",
    "cqt_to_audio",
    "cyclic_tempogram",
    "db_to_amplitude",
    "db_to_power",
    "decompose",
    "decompose_stems",
    "decompose_stems_linked",
    "decompose_with_init",
    "deemphasis",
    "detect_beats",
    "detect_acoustic",
    "detect_bpm",
    "detect_boundaries",
    "detect_chords",
    "detect_downbeats",
    "detect_key_candidates",
    "detect_key",
    "detect_onsets",
    "ebur128_loudness_range",
    "engine_abi_version",
    "estimate_meter",
    "controller_profile_names",
    "synth_gs_drum_kit_is_voiced_apart",
    "synth_gs_drum_kit_name",
    "synth_gs_variation_is_voiced_apart",
    "project_abi_version",
    "synth_enum_tables",
    "synth_preset_names",
    "synth_preset_patch",
    "estimate_tuning",
    "voice_changer_abi_version",
    "fix_frames",
    "fix_length",
    "fourier_tempogram",
    "frame_signal",
    "frames_to_samples",
    "frames_to_time",
    "harmonic",
    "has_ffmpeg_support",
    "hpss",
    "hpss_with_residual",
    "hybrid_cqt",
    "hz_to_mel",
    "hz_to_midi",
    "hz_to_note",
    "lufs",
    "lufs_interleaved",
    "lufs_series_interleaved",
    "mel_spectrogram",
    "mel_delta",
    "reassigned_spectrogram",
    "cross_similarity",
    "recurrence_matrix",
    "recurrence_to_lag",
    "lag_to_recurrence",
    "subsegment",
    "agglomerative",
    "path_enhance",
    "griffin_lim",
    "mel_to_audio",
    "mel_to_hz",
    "mel_to_stft",
    "master_audio",
    "master_audio_stereo",
    "mastering",
    "mastering_ab_match_loudness",
    "mastering_assistant_suggest",
    "mastering_assistant_suggest_chain",
    "mastering_assistant_suggest_chain_stereo",
    "mastering_assistant_suggest_stereo",
    "mastering_audio_profile",
    "mastering_audio_profile_stereo",
    "mastering_chain",
    "mastering_chain_stereo",
    "capability_catalog",
    "mastering_dynamics_compressor",
    "mastering_dynamics_gate",
    "mastering_dynamics_transient_shaper",
    "mastering_insert_names",
    "mastering_insert_param_info",
    "mastering_insert_param_names",
    "mastering_insert_timing",
    "mastering_pair_analysis_names",
    "mastering_pair_analyze",
    "mastering_pair_process",
    "mastering_pair_processor_names",
    "mastering_platform_names",
    "mastering_preset_names",
    "mastering_preset_params",
    "mastering_process",
    "mastering_process_stereo",
    "mastering_processor_catalog",
    "mastering_processor_names",
    "mastering_repair_declick",
    "mastering_repair_declick_stereo",
    "mastering_repair_declip",
    "mastering_repair_declip_stereo",
    "mastering_repair_decrackle",
    "mastering_repair_decrackle_stereo",
    "mastering_repair_dehum",
    "mastering_repair_dehum_stereo",
    "mastering_repair_denoise_classical",
    "mastering_repair_denoise_classical_linked",
    "mastering_repair_denoise_classical_stereo",
    "mastering_repair_dereverb_classical",
    "mastering_repair_dereverb_classical_linked",
    "mastering_repair_dereverb_classical_stereo",
    "mastering_repair_dereverb_config_for_room",
    "mastering_repair_detect_clicks",
    "mastering_repair_detect_clipping",
    "mastering_repair_detect_crackle",
    "mastering_repair_detect_hum",
    "mastering_repair_detect_noise_floor",
    "mastering_repair_detect_reverb",
    "mastering_repair_detect_trim_range",
    "mastering_repair_detect_trim_range_stereo",
    "mastering_repair_noise_band_bins",
    "mastering_repair_trim_silence",
    "mastering_repair_trim_silence_stereo",
    "mastering_stereo_analysis_names",
    "mastering_stereo_analyze",
    "mastering_streaming_preview",
    "mastering_streaming_preview_stereo",
    "metering_crest_factor_db",
    "metering_crest_factor_db_stereo",
    "metering_dc_offset",
    "metering_detect_clipping",
    "metering_dynamic_range",
    "metering_peak_db",
    "metering_phase_scope",
    "metering_phase_scope_decimated",
    "metering_rms_db",
    "metering_silence_ratio",
    "metering_spectrum",
    "metering_spectrum_frame",
    "metering_stereo_correlation",
    "metering_stereo_width",
    "metering_true_peak_db",
    "metering_vectorscope",
    "metering_vectorscope_decimated",
    "waveform_peak_pyramid",
    "waveform_peaks",
    "mfcc",
    "mfcc_to_audio",
    "mfcc_to_mel",
    "Mixer",
    "MixerStereoResult",
    "mix_source_class_from_name",
    "mix_source_class_names",
    "mix_stereo",
    "mixing_scene_preset_json",
    "mixing_scene_preset_names",
    "suggest_mix_scene",
    "suggest_mix_scene_json",
    "midi_to_hz",
    "momentary_lufs",
    "nn_filter",
    "nnls_chroma",
    "normalize",
    "normalize_rms",
    "normalize_stereo",
    "normalize_rms_stereo",
    "note_stretch",
    "note_move",
    "note_segments",
    "extract_notes",
    "render_notes",
    "decompose_note_pitch",
    "split_note",
    "merge_notes",
    "note_targets_from_smf",
    "assign_note_targets",
    "NoteEdit",
    "NoteObject",
    "NoteTarget",
    "PitchDecomposition",
    "transcribe",
    "TranscribeResult",
    "PolyphonicAnalysis",
    "extract_percussive_events",
    "render_percussive_events",
    "PercussiveEvent",
    "PercussiveEventEdit",
    "onset_backtrack",
    "note_to_hz",
    "onset_envelope",
    "onset_strength_multi",
    "pad_center",
    "pcen",
    "peak_pick",
    "percussive",
    "phase_vocoder",
    "plp",
    "pitch_correct_timevarying",
    "pitch_correct_to_midi",
    "pitch_correct_to_midi_timevarying",
    "pitch_pyin",
    "piptrack",
    "pitch_shift",
    "pitch_tuning",
    "pitch_yin",
    "poly_features",
    "preemphasis",
    "pseudo_cqt",
    "power_to_db",
    "remix",
    "remix_aligned_intervals",
    "resample",
    "rms_energy",
    "spectral_bandwidth",
    "spectral_centroid",
    "spectral_contrast",
    "spectral_edit",
    "spectral_flatness",
    "spectral_flux",
    "spectral_rolloff",
    "SpectralRegionOp",
    "stft",
    "stft_db",
    "samples_to_frames",
    "scale_correction_semitones",
    "scale_pitch_class_enabled",
    "scale_quantize_midi",
    "short_term_lufs",
    "split_silence",
    "split_silence_common",
    "split_silence_common_with_report",
    "tempogram",
    "tempogram_ratio",
    "time_stretch",
    "time_to_frames",
    "tonnetz",
    "trim",
    "trim_silence",
    "vector_normalize",
    "voice_change",
    "RealtimeVoiceChanger",
    "RealtimeVoiceChangerConfig",
    "StreamingRetune",
    "voice_change_realtime",
    "realtime_voice_changer_preset_config",
    "realtime_voice_changer_preset_json",
    "realtime_voice_changer_preset_names",
    "realtime_voice_changer_preset_pod",
    "validate_realtime_voice_changer_preset_json",
    "voice_character_preset_id",
    "version",
    "vqt",
    "vqt_to_audio",
    "zero_crossing_rate",
    "zero_crossings",
]
