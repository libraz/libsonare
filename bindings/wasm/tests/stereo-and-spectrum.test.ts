/**
 * Tests for the offline stereo / phase-scope / spectrum WASM wrappers.
 *
 * These functions are thin embind shims over the metering/* modules. The
 * goal is to verify the marshaling: scalar returns are numbers, point series
 * come back as Float32Array pairs, and the documented defaults match.
 */
...
describe('Stereo meter wrappers (WASM)', () => {
  beforeAll(async () => {
    await init();
  });
...
describe('Spectrum meter wrapper (WASM)', () => {
  beforeAll(async () => {
    await init();
  });
