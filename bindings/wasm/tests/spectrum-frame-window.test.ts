...
import { init, meteringSpectrum, meteringSpectrumFrame } from '../src/index';
...
describe('meteringSpectrumFrame validates only the analysis frame', () => {
  ...
  it('refuses a fractional frameOffset as an offset, not as a bad sample', () => {
    // A fractional index reads `undefined` out of the buffer, which a scan that
    // used the offset unfloored reported as a non-finite sample that is not
    // there -- refusing the call for the wrong reason, at the wrong layer.
    let caught: unknown;
    try {
