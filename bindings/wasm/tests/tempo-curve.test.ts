...
describe('analyze() tempo curve', () => {
  ...
  it('withholds the curve until it is asked for', () => {
    ...
    const withCurve = analyze(samples, SR, { computeTempoCurve: true });
    expect(withCurve.beatLocalBpm).toHaveLength(withCurve.beats.length);
    expect(withCurve.beatLocalBpm.length).toBeGreaterThan(0);
    for (const bpm of withCurve.beatLocalBpm) {
  ...
  it('adds an output without moving the analysis under it', () => {
    ...
    expect(withCurve.bpm).toBe(without.bpm);
    expect(withCurve.beats.length).toBe(without.beats.length);
    expect(withCurve.timeSignature).toEqual(without.timeSignature);
  });
