...
**The WebAssembly package is larger than the last release, and carries two size reductions.** The net figure first, because it is the one a browser pays: measured against the published 1.7.2 binaries, the full module goes from 4.16 MB to 5.20 MB raw and from 1.41 MB to 1.73 MB compressed, so a page that loads it transfers about 320 kB more. That is what the mixing assistant, the stem separator, the thirty GS variation voices and the re-fitted piano cost. The analysis-only module goes the other way where it counts: 966 kB to 1.00 MB raw, but 381 kB to 374 kB compressed, so its download is smaller than 1.7.2's. The two reductions below are what came *off* during the cycle, not net changes.
...
**Almost every parameter in the catalog now carries its type, its default and its accepted range.** All 1,130-odd parameters across the 88 named processors reported `null` for `default`, `min` and `max`, which is the one thing a host needs to build a control surface from the catalog. Five still do, all of them on `saturation.multibandExciter`, whose descriptor publishes its keys without the band prefix its builder reads them under — so nothing measures them and their type falls back to the neutral one rather than the one that was read. `sonare_mastering_insert_param_info` and `sonare_capability_catalog_json` now answer with real values, as do `masteringInsertParamInfo` / `capabilityCatalog` on Node and WASM and `mastering_insert_param_info` / `capability_catalog` on Python. The Node and WASM `MasteringInsertParamInfo` types declared only `name` / `id` / `rtSafe` / `unit?`; they now declare the whole descriptor, and `unit` is `string | null` rather than optional.
