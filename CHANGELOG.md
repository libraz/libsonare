...
The corrective EQ is deliberately reluctant. A band is carved only where one part is built around it and the other can spare it, measured as each track's share of its own energy there — two parts colliding in a band they are both made of get nothing, because there is no version of that cut which does not take the foundation out of one of them. When a cut is made, its centre frequency is measured inside the band rather than taken from the band's midpoint: a band runs up to two octaves, so its centre can sit an octave away from the overlap the cut was justified by. The written reason names the frequency, says whether it was measured or fell back to the grid, and gives both parts' shares.
...
**Behaviour change: vocal and lead are named, not measured.** No measured feature separated a sung voice from a pad, a synth lead or a wind line, and the vocal rule gave a synthetic pad the confidence it gave a choir, so the assistant treated pads as vocals. Both classes now come only from a track's name, as keys and strings already did, and an unnamed voice, pad or lead stays `unknown`. A name now redirects the class when the measurement does not contradict it, rather than only lowering the measured class's confidence, and a new `drumKit` class covers a whole kit on one track, from its name or from hits split between the low and high bands. A track whose energy sits mostly below the mono crossover is centred whatever its class.
### Mastering

**Most masters this release produces differ from the last one's.** Twenty-two of the twenty-five presets v1.7.2 shipped hash differently against the same input, and five restoration presets are new. The changes below are why; each is called out separately because they reach different material.

**The loudness stage drives the limiter toward its target instead of giving up in front of it.** It capped its gain at the headroom between the ceiling and the input peak, so on peak-normalized material — the ordinary case — the gain collapsed to nothing however far away the target was, and the true-peak limiter downstream, the stage that exists to make a target reachable, was never driven at all. Two presets an octave apart in loudness produced the same master: on one mix `kpop` and `pop` landed 0.74 LU apart, which nobody can hear. The stage now applies the full difference to the target and lets the limiter bring the signal back under the ceiling, bounded by `maxLimiterGainReductionDb` — 12 dB by default, because every preset reaches its full requested gain there and nothing above it moves a measurement. The same mix now separates by 5.05 LU, in the right order. The allowance never permits more gain than the target asks for, so it cannot make a master louder than its preset; it decides how peaky an input the stage will still try to normalize. Setting it to 0 restores the previous clamp exactly. What shortfall remains on the loudest targets is the limiter's own gain reduction, which one non-iterating pass does not re-measure, and is reported through the existing `loudnessTargetLimited` result flag. Each preset's allowance is listed as `maxLimiterGainReductionDb` in the capability catalog's `masteringPresets` (below).

**The true-peak limiter enforces its ceiling per sample rather than per block, so its output no longer depends on the host's buffer size.** The decimation guard rescaled a whole block by one factor derived from that block's own reconstructed peak. Measured on one buffer with only the block size varied, results spread up to 0.068 absolute, about 22.8 dB below peak; below roughly 14 dB of drive only the final samples moved, above it the whole buffer shifted. The streaming chain drives the same limiter, so a streaming render at 512-sample blocks and an offline render of the same material already disagreed by that much on a heavily limited signal. Output is now identical across block sizes from 256 to 16384, whole-block and ragged splits alike, at drives from 0 to 24 dB. The cost is that a block's measured true peak is no longer forced exactly onto the ceiling: a meter running finer than the limiter reads about 0.02 dB over it, flat in drive. That guarantee was never absolute — the previous 512-block case already read over at eight-times oversampling — and the header now records the residue with its conditions.

**The tape and transformer hysteresis stays stable at the drive it documents.** The Jiles-Atherton update is a fixed-point iteration that diverges once the drive field moves faster than the coercivity, ratcheting the magnetization into its safety clamp, where the output stops following level. Adaptive sub-stepping covered the default drive, but the sub-step ceiling left the per-step ratio unbounded again: 16 kHz at +24 dB asks for roughly 300 sub-steps, and the tape stage documents a recommended range up to +24 dB, so this sat inside what it claimed to support. The increment is now projected onto the anhysteretic target so one step cannot cross the curve, and the reversible term is integrated in closed form. Measured at +24 dB and amplitude 1.0, the peak moves from 1.2013 to 0.9821 at 6 kHz, 1.1784 to 0.9652 at 12 kHz and 1.1806 to 1.0116 at 16 kHz; before, a 6 dB amplitude change moved the peak by less than 0.04, which is what a pinned output looks like from outside. A second fix keeps a high reversibility coefficient from ratcheting into the clamp by the same route.

**Five saturation processors gain a 4x oversampled path**, with preallocated scratch, dry-path delay alignment and a reported latency: the hard clipper, soft clipper, waveshaper, exciter and presence enhancer. An ADAA mode a processor does not implement is refused rather than silently ignored. The equalizer snapshot's `profileDb` is computed from a windowed FFT summed into geometric bands instead of estimated from the waveform stream, and match-EQ band gains are solved against the composite response of the whole band set, so overlapping peaking bands no longer stack past `maxGainDb`.

**`repair.denoise.gainFloor` is now `repair.denoise.reductionDb`, and says the same thing the other way up.** The knob was a linear floor; it is the deepest attenuation the gain mask may apply to any bin, in dB, defaulting to 26. A document written against the old spelling still applies: the retired key is read and converted, and the conversion carries the old validity range with it, so a floor above unity becomes a negative depth and is still refused. The flat `repair.reductionDb` alias joins the other repair shorthands. The default floor moves from 0.05 to 10^(-26/20), so a denoise-enabled preset renders slightly differently.

**The repair stages run widest damage first.** Declipping now precedes declicking, because a flat-topped region carries no transient for a click detector to measure, and dereverberation now follows denoising, because a broadband floor reads as a stationary late tail and pulls the reverb estimate toward it. The mono and stereo paths spelled the order independently and only the mono one was reachable from the preset goldens, so both are now pinned to the same sequence rather than leaving the stereo render free to diverge by channel count.

**Five restoration presets are new**, for damaged source material: `vinyl`, `tapeHiss`, `fieldRecording`, `voiceMemo` and `shellac78`. Each enables repair stages only and leaves level untouched, so none of them carries a loudness target. `shellac78` takes a wider eligible click run and a lower crackle threshold for a coarser groove, and leaves its residual noise floor deeper. The names reach the Node, WASM and Python type surfaces and the capability catalog.

**The capability catalog describes each mastering preset, and a preset's flat parameters can be read.** `masteringPresets` is a new top-level catalog key, one entry per preset in `presets.mastering` order: `{ name, kind, targetLufs, truePeakCeilingDb, maxLimiterGainReductionDb }`, where `kind` is `"mastering"` or `"restoration"` and a restoration preset reports `null` for the three numbers, because it leaves level alone. `presets.mastering` stays the plain name list. `sonare_mastering_preset_params_json` returns the flat parameter map a preset expands to, in the same key space `masteringAssistantSuggestChain` returns and `overrides` accepts, so a host can keep only the keys where a suggestion differs from its preset. `masteringPresetParams` on Node and WASM, `mastering_preset_params` on Python. The WASM package now exports `CapabilityCatalog` and its part types — `CapabilityCatalogProcessor`, `CapabilityCatalogParameter`, `CapabilityCatalogPresets` and `CapabilityCatalogMasteringPreset` — from its entry, under the same names Node uses, so a host can name them instead of deriving them from `capabilityCatalog`'s return type.

**Behaviour change: any one key of an EQ band creates the band, and the catalog says which keys belong to a conditional group.** The EQ families disagreed about what makes a band exist: `eq.parametric` and `eq.equalizer` built it for any of its keys, `eq.minimumPhase`, `eq.linearPhase` and `eq.midSide` only for `frequencyHz` or `gainDb`, and `eq.dynamic` and the dynamic sub-bands of `multiband.dynamicEq` only for `frequencyHz`, reporting the band's other keys as unread. All of them now build a band for any one of its keys, so a configuration whose band keys were all read before builds the same processor, and one with band keys reported as unread now builds the bands they address. Every catalog parameter carries `slot`, the conditional group it belongs to (`"midBand3"`, `"band1.dyn2"`, or null), and every processor entry carries `slots`, one `{ name, parent, activation, minCrossoverCutoffs }` entry per group in declaration order: `activation` is `"anyKey"` for a band any of its keys creates and `"always"` for a crossover band, which exists once the crossover in effect has `minCrossoverCutoffs` cutoffs. This reaches `sonare_mastering_insert_param_info`, `sonare_mastering_processor_catalog` and the capability catalog on every surface, and `MasteringInsertSlot` is the new type on Node, WASM and Python. The multiband processors now list every band up to the nine a crossover can split into; the catalog stopped at the three of the default split, though construction accepted the rest.

**A restoration preset can remove a steady tone.** Its repair stages estimate noise from what stays constant, so a calibration tone, a drone or a long held note is learned as noise and pulled down by the configured reduction depth; on a five-second 440 Hz tone the output lost about 26 LU. Material with gaps between its notes keeps its level. This is how the classical denoiser works rather than a regression, and the preset documentation on every surface, and the denoise estimator's, now say so. Check for sustained musical tones before applying one.

**An EQ band can be an all-pass.** `AllPass` joins the band types on every surface: unit magnitude at every frequency, with a phase rotation around the band's frequency, which is what lines up two sources that cancel each other there. A magnitude display shows nothing for it. The linear-phase equalizer refuses it, because it has no phase to rotate.

**The mastering assistant takes a delivery target.** `targetPlatform` names one of the targets `sonare_mastering_platform_names` lists (`broadcast`, `podcast`, `club` and the rest). On Node, WASM and Python it is the name itself, and a number is refused; the C ABI carries an index, looked up with `sonare_mastering_platform_from_name`, because a C param value is a number. This is why the assistant's params type on Node and WASM is now `MasteringAssistantParams`, which admits a string value. Every call that compiled before still compiles.
...
`mel_spectrogram`, `mfcc`, `stft`, `chroma`, `chroma_cens`, `chroma_cqt` and `bass_chroma` now check their buffer before the call instead of relying on the native side, so an empty or non-finite one is reported against the function the caller invoked rather than against an internal helper or a C symbol. A buffer shorter than the analysis window is not a case any of them refuses, here or before: it is framed and padded as it always was. `zero_crossings` and `pitch_tuning` reject empty and non-finite input, which they previously accepted and answered with a fabricated result.

**The exception type and message text change for roughly a hundred and twenty buffer-taking entry points.** Every public function whose leading argument is a sample buffer now preflights it, so the analysis, mastering, repair, spectral-feature, constant-Q and segmentation entry points raise `SonareValueError` naming the function and the offending argument — `master_audio_stereo: right must not be empty`, `spectral_centroid: samples contains NaN or Inf at index 0` — where they previously let the call reach the C ABI and surfaced the bare `SonareError: [4] Invalid parameter`. `SonareValueError` subclasses both `SonareError` and `ValueError` and carries `ErrorCode.INVALID_PARAMETER`, so a caller catching either base class or branching on `.code` is unaffected; a caller matching on the message text is not. `cross_similarity` reports its own `x` / `x_rows` parameter names rather than the internal `data` / `rows` spelling, and the scalar and stereo meters name the facade function instead of the `sonare_`-prefixed C symbol.

Five entry points reject input they previously accepted. `trim_silence`, `split_silence` and `fix_frames` raise on an empty buffer instead of returning an empty result, and `tempogram_ratio` rejects an empty `tempogram_data`, as well as a `factors` entry that is not finite and positive — a NaN there reached an undefined float-to-int conversion in the core and an infinity silently degenerated to the DC lag. `mix_stereo` rejects a scene it cannot mix: strips that are all empty previously answered with an empty result and per-strip meters reading the `-120` dB floor, which is what a real mix of silence looks like, and a strip carrying NaN metered as that same floor because a NaN loses every comparison the peak meter makes. Both now raise. The NaN names the strip index and the channel it was found in; a scene whose strips are all empty says so without naming one, because there is no offending strip to point at. A zero-frame block stays a valid no-op at the C ABI, where it means "process this block" rather than "mix these strips". Element-wise conversions keep their empty-in / empty-out contract and are unchanged: `power_to_db`, `amplitude_to_db`, `db_to_power`, `db_to_amplitude`, `preemphasis`, `deemphasis`, `vector_normalize`, `frame_signal`, `pad_center`, `fix_length`, `tempogram`, `cyclic_tempogram` and `fourier_tempogram` — but not `tempogram_ratio`, which is one of the five above. `f0_hz` is likewise never scanned for non-finite values, because pYIN marks an unvoiced frame with NaN and that track representation must pass straight into pitch correction and note rendering. `render_notes` was the one entry point that did scan it, so the track its own documentation tells the caller to hand it — the one `extract_notes` had just accepted — came back refused; it now reads the argument for shape alone, like its five siblings.

**The feature ABI moved from 4 to 5.** `SonareNoteSegmenterConfig` gained a trailing `voiced_threshold` field, so the Python binding refuses a shared library built from a different tree; rebuild the library alongside the binding. Existing C callers are otherwise unaffected at both the source and the call level: the new field is read only when `struct_version` is 2, so a struct filled as before keeps its previous behaviour. **The acoustic ABI moved from 3 to 4** for the same reason, and carries the same obligation: `SonareRirSynthConfig` and `SonareRoomMorphConfig` each gained three trailing atmospheric-absorption fields. Both versions are packed into the one aggregate a binding checks, so either move alone makes it refuse a stale shared library. **The project ABI moved from 1 to 2**, because `SonareProjectClipCompSegment` gained a `crossfade_ppq` field and so grew from 24 bytes to 32. Unlike the two above this one reaches a released layout — v1.6.0 through v1.7.2 all shipped the 24-byte struct — so a consumer holding a header from any of them must be rebuilt rather than merely relinked, and the version is what stops it reading a comp lane shifted by a field. The engine and voice-changer ABIs are unchanged.
...
The built-in mastering preset golden hashes are refreshed for the six presets that enable the air band, separately from the behaviour changes above. Those hashes still described the output from before the air band's dry and harmonic paths were time-aligned and before the offline loudness stage was measured from its own input; both of those shipped in earlier releases without the fixture being regenerated, so refreshing them corrects the fixture rather than recording a change.

### CLI

**`mastering-processor` no longer has a `--stereo` flag, which is a breaking change for anyone who spelled it.** It is not needed: a two-channel file now takes the stereo path on its own, and a processor with no mono implementation takes it from any input. Both front-ends removed it together. What the flag did until now was select a code path, not a signal — and the signal it selected was wrong, which is the second half of this entry.

**`mastering-processor` now processes the two channels a stereo file actually carries, and writes two channels.** It previously loaded the mono downmix, handed that one buffer in as *both* channels, folded the processed pair back down and wrote one channel, while reporting `"stereo": true`. Three of the stereo processors were therefore inert — `stereo.imager`, `eq.midSide` and `stereo.monoMaker` each returned the untouched downmix, byte for byte — and `stereo.stereoBalance` was worse than inert, since it did change the audio, as a function of a fabricated image the caller never supplied. Output is now two channels wherever the stereo path runs, which changes the shape of the file this command writes. Whether a given processor links its decision across the pair or runs per channel is the library's own choice and is unchanged.

**`normalize` carries a stereo file through as a pair instead of downmixing it.** One gain is measured across both channels and applied to both, so the louder channel reaches the target and the quieter one keeps its level relative to it — normalizing each channel to the target separately is a balance change rather than a level change, and would collapse the image toward the centre. A mono input still produces a mono file, and the JSON payload is unchanged: `length` is per channel, and no channel count is reported, because that is a property of the file.
...
**Four commands that existed only on the Python CLI now exist natively as well.** `transcribe` writes a Standard MIDI File from audio, `midi-render` renders a MIDI project through the synthesizer, `decompose-stems` separates a signal into listenable components, and `suggest-mix` measures a set of tracks and prints a suggested mixer scene. All four take the same options and report the same exit codes as their Python counterparts, and what they write matches — the component stems, the bounced audio on each preset and the MIDI file are byte-identical, and the suggestion document is the same document.

`suggest-mix` reads a two-channel input as a stereo pair rather than folding it, because the image analysis is the one part of the assistant that measures both channels and a folded stem would have it describe a signal the caller never has. An unrecognised `--params` key is refused on both front-ends: the config builder ignores one by design, so a typo there would otherwise run to completion having changed nothing.

**`mix-strip` now exists on the Python CLI too, and both front-ends write the same bytes.** It was the last command either front-end carried alone for want of a port. The two produce byte-identical output across the option space, including a mono source and the refusal of `--width` on one.

**`repair` no longer discards the repair it just made.** Declipping rebuilds the peaks a clipper cut off, so its output routinely exceeds full scale, and this command deliberately runs no limiter — the integer writer then clamped, pinning exactly the samples the repair had rescued back onto the ceiling they came from. On a fifth-clipped file, 8446 of the input's 8672 full-scale samples were still full-scale in the output, so 97 % of the reconstruction was thrown away at the last step. The output is now fitted to full scale with one gain for the whole file, which keeps the reconstructed waveform's shape, and the applied gain is reported as `output_gain_db` in `--json` and as a line in the text report. The key is always present and is `0` when the peak already fit, so a file that did not clip is written exactly as before.

**`project bounce --synth` spelled bare, and `midi-render` with no `--synth` at all, send the same synth patch from both front-ends.** `midi-render` always renders through the synthesizer, so its `--synth` names a preset and takes a value on both front-ends; only `project bounce` treats the bare flag as a request. They did not agree on what that patch was: the native CLI named the `sine` preset as the patch's base while the Python CLI sent the init patch, asserting thirty field values the caller never chose. Nothing rendered differently, because the bare flag also turns on GM program routing and that replaces the patch at every note-on — but the two front-ends disagreed about what would happen if it ever did not. The native side now sends the init patch, which is what the empty preset name already means. No rendered output changes.

**`project bounce` can bind audio to a project's sources.** A project document names its audio by URI and the library never opens one, so the command line is the host that has to decode a file and hand the samples over — and until now it had no way to. A project carrying audio clips reached the render as a bare invalid state. `--audio <source_id>=FILE` binds one decoded file per occurrence, repeatable; `--resolve-audio` opens the `file://` URIs the document already carries, refusing any other scheme by source id and URI rather than skipping it. A source still unresolved is named with its id, its URI and the option that would have supplied it. Both front-ends take both options, and a bound take keeps its channel count. `midi-render` declares neither and is unchanged, so its error does not point at flags it does not have.

**`split-silence` now cuts several takes of one part at the silence they share, and it exists on the Python CLI too.** It had been native-only on the grounds that the measurement was already a Python entry point, so a command would add only a round trip — true of one signal, and not of several, because passing several files on one command line is the thing a command does that an array a caller already holds does not. `--input` is repeatable and names further takes; the positional stays the first. The reported intervals are the union of each take's own, merged where they touch, so a cut falls only where every take is quiet and lands mid-phrase in none of them, and one take answers exactly as before. `--write-takes PREFIX` writes every take sliced at every interval as `PREFIX{take}_{interval}.wav`, both indices 1-based: a take that ended before an interval is silent there, so its slice is padded rather than shortened and one interval's files are the same length across takes, which is what lets them be compared. A take whose sample rate differs from the first is refused by path and by both rates, because frame indices from two rates do not describe the same positions. Both front-ends take the same options and refuse the same conditions with the same exit codes.

**`tune-to-midi` is new, on both front-ends.** It tunes a take to a melody that already exists as a MIDI file: `tune-to-midi take.wav --reference-smf melody.mid -o tuned.wav` measures the take's pitch, matches each note it finds to the reference note it overlaps, and renders the corrections. `--reference-smf` is required. `--track N` indexes the tracks that carried MIDI events rather than the file's own numbering, so a file whose first track is a conductor track has its melody at 0. `--unmatched-policy {leave,mute,nearest}` decides what becomes of a note the reference does not reach — left as recorded, silenced, or corrected to the nearest target in time. `--min-overlap-ratio` and `--max-correction-semitones` carry no command-line default, because 0 is a legal value for both — any overlap at all, and a correction that saturates to nothing — so an absent flag has to stay distinguishable from a zero and leave the library's own value in place. `--json` reports `assigned_count` beside `note_count`: a reference that does not line up with the take assigns nothing and still succeeds, and those two numbers are the only thing that says so. A value outside a bound, an unknown policy name, a negative track index and a missing `-o` are all invalid parameters; a missing `--reference-smf` is a usage error, because both front-ends require it in the parser; one that cannot be opened is a file-not-found. The native binary needs both the arrangement and pitch-editor subsystems — the reference reader belongs to the first and the assignment rule to the second — so a build without either does not list the command at all.

**`split-silence --report` says why those are the intervals, on both front-ends.** One interval covering everything is the answer to three different situations and the intervals separate none of them: no take has a quiet moment, the takes each have one but never in the same place, or `--top-db` was set too loose to see the ones they have. The flag adds the largest `--top-db` at which every take still shows silence, read against the one in use: near zero a take is sounding continuously and no threshold helps, below the one in use the threshold was too loose and one under the reported figure finds the quiet, and at or above it the quiet is there and the takes do not share it. It wraps the result rather than replacing it — without the flag the `--json` payload is the bare interval array it has always been, and with it the payload is `{intervals, report}` — so nothing that reads the array by index has to change. Text mode prints the same two figures.

**`project align-takes` is new, on both front-ends.** It aligns every take in a project document against one named reference source and writes the document back with the alignment in it: `project align-takes --in take-comp.json --reference-source 1 --audio 1=lead.wav --audio 2=double.wav -o aligned.json` measures each take against the reference, registers the result as a project warp map named `take-<source id>`, points every clip on that source at it, and sets those clips to the time-stretch warp mode so the document plays aligned. A take is an audio source other than the reference that at least one clip references; a source no clip reaches is not aligned and needs no file. New warp maps take the ids after the highest already in the document, in ascending source order, so nothing existing is renumbered. `--audio <id>=FILE` and `--resolve-audio` mean exactly what they mean on `project bounce`, because a project names its audio by URI and the library never opens one — but this command decodes the files itself rather than binding them, so it wants the paths and nothing else. `--hop-length` sets the frame rate the drift is measured at and `--bins-per-octave` the chroma resolution; both read 0, which is also what an absent flag sends, as the library value, and a resolution the measurement cannot fold onto the twelve pitch classes is refused by the library rather than by the command line. The two recordings must share a sample rate, and a pair that does not is refused naming both rates. `--json` reports each take's source, its new warp map, the two frame counts and the mean residual, so a take that drifts badly is visible without opening the document. A document with no take to align is an invalid parameter rather than a success that wrote nothing. Both front-ends write byte-identical documents. The native binary needs the arrangement subsystem, so a build without it does not list the command.

### Build configuration

**The capability descriptor names every build option that changes what a binary answers.** `features` in `sonare_capabilities_json` (and so in `doctor --json`, and in `capabilities()` on all three bindings) gains `arrangement`, `acousticSim`, `pitchEditor` and `voiceChanger` beside the six it already carried. It previously reported six of the nine gates, and the three it could not name were exactly the ones a caller had no other way to detect: a command answering "not compiled in" was indistinguishable from one that was never implemented.

The cross-surface CLI contract reads that descriptor and no longer demands commands from a build that does not have them. A feature-reduced native binary previously failed the contract on every command behind a gate it was built without — 113 failures on a `-DBUILD_PITCH_EDITOR=OFF` build — which meant the contract could only ever be run against a default build, and no CI job configured any other. Each front-end is now asked about its own library, because the two do not have to load the same one.
...
- Warped clips can follow their warp map without transposing. `'time-stretch'` joins `'off'`, `'repitch'` and `'tempo-sync'` as a clip warp mode on both the engine and the project surfaces. It reads the same anchor map as `'repitch'` but synthesizes the output by overlap-adding source segments at a fixed rate, so changing the map moves the timing and leaves the pitch where it was — and it takes effect from the next block, with no control-thread bake to redo. The stretcher's voices are preallocated; a clip that cannot get one falls back to `'repitch'` behaviour rather than allocating on the audio thread. The pool holds 8 voices by default, and `setWarpVoiceCapacity` (`set_warp_voice_capacity` on Python, `sonare_engine_set_warp_voice_capacity` on the C ABI) sets it anywhere from 0 to 64 (`SONARE_ENGINE_MAX_WARP_VOICES`); `warpVoiceCapacity` reads it back. Capacity 0 turns stretching off, and those clips do not count as overflow. A change while the engine runs takes effect at the next block and restarts the splice state of any clip stretching at that moment.
- `parameterInfo` describes the engine's reserved automation ids, not only the parameters a host registered. A hosted-instrument parameter (from `resolveInstrumentAutomationId`), a lane or bus fader, a lane pan, the master fader, pan and width, and an insert on a strip built with `setTrackStrip`, `setBusStrip` or `setMasterStrip` each report a name, range, default and unit. An instrument reports the defaults it is built with, not the patch currently loaded, and a layered instrument prefixes each name with its layer index (`"0.cutoffHz"`). An insert's range comes from the insert catalog, where a `null` bound is reported as an infinity. The master fader has no clamp, so it reports an unbounded range. An insert on a strip bound directly, with no spec behind it, is still an invalid parameter, as is an id nothing resolves. `parameterCount` and `parameterInfoByIndex` still list registered parameters only.
...
- A synthesized room can absorb like air. `airAbsorptionEnabled` / `air_absorption_enabled` adds the ISO 9613-1 atmospheric-absorption term to the late tail's per-band RT60, which mainly shortens the high bands of a large room and leaves a small one close to where it was. It reaches `synthesizeRir` and `roomMorph` on all four surfaces and the `effects.reverb.room` and `effects.acoustic.roomMorph` inserts. It is off by default, so a room built as before renders as before. The climate is `airTemperatureC` and `airHumidityPercent`, both following this ABI's rule that 0 selects the library value — here the ISO reference climate, 20 degC at 50 % relative humidity — so a freezing room is requested as 0.01 rather than 0, which absorbs identically. An implausible pair is reported the way the surrounding geometry checks are rather than clamped.
- `sonare_strip_settle` snaps a mixer strip's input-trim, fader, pan and width smoothers to the values already set on it, so the next processed block opens at those values instead of gliding to them over the smoothing window. A strip is smoothed for a live fader; an offline render of a finite buffer that does not settle carries that glide as a level and image sweep across the head of its output — around 2560 samples at 48 kHz, far longer than the 5 ms time constant suggests. `settle` on Node, WASM and Python, taking the strip the way its `setWidth` sibling does on each surface. It clears nothing: automation, meters and insert state are untouched, and the scene is unchanged.
- The mastering assistant's suggestion is reachable as a chain configuration on its own. The fuller proposal document wraps that configuration in an explanation, genre candidates and a profile, so a caller that wants to apply a suggestion has to dig it back out. `sonare_mastering_assistant_suggest_chain_json` and its stereo sibling write the configuration alone, and each binding unwraps the `{version, params}` envelope and returns the flat parameter mapping, which goes straight back in as `overrides`: `masteringAssistantSuggestChain` on Node and WASM, `mastering_assistant_suggest_chain` on Python, each with a stereo counterpart that profiles the pair rather than a downmix. One analysis pass, as the fuller endpoint takes. A parameter value that is neither a number nor a boolean, and an empty parameter map, are refused by name rather than dropped.
- `sonare_split_silence_common` finds the cut points several takes of one part agree are silent. Comping a part means cutting every take at the same places, and what the takes share is the silence rather than the sound: a take that happens to be quiet where another is still singing must not be cut there. The result is the union of what `sonare_split_silence` reports for each signal, merged where the intervals touch, so every gap between the returned intervals is silent in all of them at once and a cut placed in one lands mid-phrase in none of them. A signal shorter than the longest contributes nothing past its own end, which is the same as being silent there, so takes of unequal length need no padding, and a single signal answers exactly as `sonare_split_silence` does. `splitSilenceCommon` takes a request object on Node and WASM, `split_silence_common` keyword arguments on Python; each derives the per-signal lengths from the sequence it is handed, because a caller holding a list of takes has no second place to state them. Cutting at the intervals is left to the caller, which is an array slice on all three surfaces.
- `sonare_split_silence_common_ex` says why the split found the gaps it did. One interval covering everything is the answer to three different situations and the interval list separates none of them: no take has a quiet moment, the takes each have one but never in the same place, or `topDb` was set too loose to see the ones they have. Until now a caller could only tell them apart by re-deriving the detector's own threshold rule from a second RMS pass of its own, and a disagreement between the two passes would look like a disagreement about the audio. The report carries the largest `topDb` at which every signal still shows silence, alongside how many intervals the most and the least fragmented signal produced on its own. Read that ceiling against the `topDb` in use and the three separate: near zero, a take is sounding continuously and no threshold helps; below the `topDb` in use, the threshold was too loose and one under the reported figure finds the quiet; at or above it, every take shows silence at this very setting, so a single interval means they do not share any of it — which is what `sonare_align_take_to_reference` is for. The interval counts measure shape rather than cause: a take that sounds once and stops counts one exactly as a take with no silence does, so the ceiling and not the count carries the verdict. The figures come from the same RMS pass the intervals do and cannot describe a different measurement. `splitSilenceCommonWithReport` on Node and WASM, `split_silence_common_with_report` on Python. `sonare_split_silence_common` is unchanged and keeps its signature, because a caller cutting takes has no use for the diagnosis.
...
- `sonare_align_take_to_reference` measures how far one take drifts from another and returns the map that puts it back. Comping several takes of one part means playing them all against one timeline, and a take sung live sits a little ahead or behind the reference throughout rather than by a single constant offset. Chroma DTW measures that drift, and the entry point returns it as project warp anchors already oriented for a clip whose source is the take — `warp_sample` on the reference timeline, `source_sample` in the take — so the result goes straight into `sonare_project_set_warp_map` and plays aligned under the `time-stretch` warp mode, with the recording itself left untouched. The anchors are reduced to the strictly increasing form a warp map requires: a DTW path holds one axis still wherever the two recordings differ in speed, and a run of tied coordinates is represented by its middle anchor, so the reduction does not pull the alignment toward either end of the run. `alignTakeToReference` takes a request object on Node and WASM, `align_take_to_reference` keyword arguments on Python. `hopLength` sets the frame rate the drift is measured at and `binsPerOctave` the resolution the twelve pitch classes are folded from, both reading 0 as the library value. A resolution that is not a positive multiple of twelve has no fold onto the pitch classes and is refused for that alone; a multiple of twelve is necessary and not sufficient, because the finest usable resolution also depends on the sample rate and does not rise with it. The two recordings are expected to share a sample rate — nothing here resamples — and a pair the measurement cannot get two distinct anchors out of, either recording shorter than one frame, is reported rather than answered with a map the project would refuse. The frame count of each side is reported beside the anchors, together with the mean absolute departure of the path from its own diagonal trend: a coarse indication of how much the two takes disagree, not an error bound. The warp map it produces belongs to the arrangement subsystem, so a build without that subsystem does not carry the entry point, and the analysis-only WebAssembly bundle does not either.
- A take can be tuned to a melody that already exists as a MIDI file, instead of to a stated interval or a scale. `sonare_note_targets_from_smf` reads one track of an in-memory Standard MIDI File as a reference melody and `sonare_assign_note_targets` writes each note's `pitch_shift_semitones` from the target it overlaps: `noteTargetsFromSmf` and `assignNoteTargets` take request objects on Node and WASM, `note_targets_from_smf` and `assign_note_targets` keyword arguments on Python. The match is temporal rather than harmonic — a note takes the target it overlaps longest provided that overlap is at least `minOverlapRatio` of its own span, half by default, and ties go to the target that starts first — and the shift saturates at `maxCorrectionSemitones`, an octave by default, rather than being refused: a reference an octave out is a wrong reference, and a bounded correction tells the caller more than a rejected call does. `unmatchedPolicy` decides what becomes of a note that has a pitch and no target: `leave` renders it as recorded, `mute` silences its span, `nearest` takes the nearest target in time however far away. A note with no measured pitch is never assigned and never edited whatever the policy says, because there is no pitch to correct from — having no pitch is a different condition from having no target. Both bounds read 0 as its own value and not as a request for the default, so `minOverlapRatio` 0 counts any overlap at all and `maxCorrectionSemitones` 0 assigns every target and moves no note. How many notes got one is reported beside them, because zero is a legitimate answer — a reference that does not line up with the take — and nothing in the notes themselves separates it from a reference that matched. The track index counts only the tracks that carried MIDI events, so a file whose first track is a conductor track has its melody at 0, and the file's own tempo map is followed rather than its initial tempo being scaled. A note-on the track never closes is dropped instead of being given the file's end, which would make it the longest overlap everywhere and take the assignment from every note after it. The SMF reader belongs to the arrangement subsystem, so a build without that subsystem does not carry `noteTargetsFromSmf`, and the analysis-only WebAssembly bundle carries neither entry point.
...
**A device gesture can be bound to what it means rather than to its controller number.** A controller binding maps one input — a CC number, channel or polyphonic pressure, pitch bend, or note-on velocity — onto one expression axis: excitation, position, brightness, morph, loudness, pitch in cents or vibrato depth. It takes a range and a curve. Binding one input twice reaches two axes, which is what a breath controller driving both excitation and loudness needs. Profiles are selected by preset name, and bindings are added on top of a profile, up to 32 per profile. `sonare_engine_bind_controller` and its siblings on the C ABI; `bindController`, `clearControllerBindings` and `controllerBindingCount` on the Node and WASM `RealtimeEngine`; `bind_controller` and `clear_controller_bindings` on Python.

**A channel can play mono, and mono can slur.** `setArticulation` chooses what a note-on does while another note on the same channel is still held: `poly` gives every note its own voice; `mono-retrigger` stops the previous note and starts over, which is all GS MONO MODE and CC126 can reach; `mono-legato` re-tunes the sounding voice without restarting its exciter or envelope. Legato is a request: a struck or plucked engine, or a pitch below what the delay line holds, falls back to an ordinary note, and `sonare_engine_legato_fallback_count` counts the fallbacks.

**MPE values reach the note they belong to.** Bend, pressure and timbre (CC#74) are tracked per note. When several notes sound on one member channel, `setControllerNoteTracking` decides which of them a channel-addressed value reaches — the last, lowest or highest note, or all of them — separately for each dimension. A released note is never chosen. The three per-note messages have pushes of their own at their full width: `pushMidiPitchBend` (14-bit), `pushMidiChannelPressure` and `pushMidiPolyPressure` on the Node and WASM `RealtimeEngine`, `push_midi_pitch_bend` and its siblings on Python, `sonare_engine_push_midi_pitch_bend` and its siblings on the C ABI. Before this release they were reachable only as a raw UMP word on WASM.

**The synth plays host-supplied PCM.** The `sample` engine mode reads a `SampleBank`: float samples plus key and velocity zones, built by the caller and bound alongside the patch. It is for a host that already has its own waveforms, so it takes no SoundFont authoring and does no decoding. The patch chooses the keymap set, the level, the loop override, a start offset and key tracking (`SonareSynthPatch` struct version 3). A bank is built on the control thread and must be complete before a render starts. `SampleBank` is a class on Node and WASM and `SynthPatch.sampleBank` holds one, so a patch that carries a bank is a native handle and cannot be stored as plain data; Python passes the bank as the `sample_bank` keyword of `set_synth_instrument`. `sonare_sample_bank_create` and its siblings on the C ABI.

**A harpsichord engine.** The `harpsichord` engine mode is a jack-and-plectrum model driving a registration of separate string choirs. Key speed moves loudness by only a few dB, as it does on the instrument, and the engine does not apply the velocity curve.

**Three more patch stages, and a wider mod matrix.** A 12 dB/oct highpass after the main filter (`hpCutoffHz`, struct version 4), a per-voice sample-and-hold and quantizer ahead of the amplitude envelope (`sampleHoldHz`, `bitDepth`, struct version 5), and a constant transposition in cents that also reaches a voice already sounding (`pitchOffsetCents`, struct version 6). The mod matrix gains the sources `breath`, `aftertouch`, `expression-cc` and `pitch-bend`, and the destinations `resonance-q`, `vibrato-depth-cents`, `filter-env-depth`, `lfo1-rate-scale`, `excitation-force`, `excitation-position`, `excitation-brightness` and `spectrum-morph`. `SynthEnumTables` lists the new names under five new tables — `controllerInputs`, `controllerAxes`, `articulations`, `mpeDimensions` and `noteTrackings` — which a Node or WASM type implementing that interface has to add.

### Acoustic piano voice

**Behaviour change, built-in synth.** The built-in acoustic grand is re-fitted against a dry concert-grand capture, and every GM program voiced from the piano model sounds different. The largest corrections are structural rather than a matter of degree:

- A soft note in the top two octaves was all but silent. The hammer-contact solver bounded the contact duration from below but not from above, so a quiet treble blow's free bounce dwelt for nearly four fundamental periods, which puts the force pulse's first null below the fundamental. A pianissimo C7 rendered 34 dB under the reference where the same note struck hard sat 12 dB under it.
- The voice carried no output level of its own and sat 16 dB under the rest of the fallback bank, so a piano in a multi-timbral arrangement was inaudible beside its neighbours.
- The bass note had almost no excitation of its own. The footprint lowpass clamped its filter coefficient at a value that is a 76.8 Hz corner at 48 kHz, which pinned every note from A0 to B2 to the same corner regardless of pitch, and the reference contact time doubled every 1.09 octaves, handing the lowest note a 13.9 ms contact whose force pulse dies at 72 Hz. The string was driven only where the soundboard cannot radiate it, and what was heard came from the hammer knock and the impact noise standing in for it. The strike point compounded this: read as a per-octave rate what a grand travels across the whole bottom of the keyboard, it landed the lowest hammer a third of the way along the string, so the comb notch that belongs on the eighth partial fell on the third.
- Longitudinal string modes are modelled. A real string's transverse motion stretches it and the tension change radiates the inharmonic partials a low piano note growls with; without them the only lever with authority in the bass was the hammer knock, which a lowpassed force confines below about 100 Hz.
- Inharmonicity follows a U-shaped curve with its minimum near C2 rather than falling monotonically, the stretch tuning is asymmetric between bass and treble as a measured Railsback curve is, and both are held at the top key rather than extrapolated past it.

Measured against the reference, the partial stack moves from 14.3 dB under it to level with it while the spectral centroid moves toward it rather than away, and the attack's 200-800 Hz band from 7.8 dB under to level. Anything relying on the previous voice's balance — a mix built around it, a bounced render, a golden hash — will differ.

### GS insertion effects

**Behaviour change, built-in synth.** A GS file's insertion-effect parameters now reach the audio. The synth received all twenty of them, held them and read them back faithfully, and then handed the insert its own compile-time default — so a Rate, Time, Depth, Level or Freq a file wrote changed nothing at all on all but four of the sixty-four effect types. Fifty-six of the parameter positions a measured unit gives a conversion to now translate onto a control of the same physical unit. The twenty-nine that remain are still held and not sounded, because the insert they map to has no control of that kind; they are counted as such rather than left as a remainder, and a position with no measurement behind it keeps the insert's default instead of being given a plausible conversion.

The conversions are measured rather than read off the printed curves, and where the two disagree the measurement is what ships: a third-octave frequency entry that answers 279 Hz where 315 is printed, a rotary speed switch that turns over one step below its printed value, a gain window narrower than the range printed against it, and a level curve that is not the curve beside it. They are the answers of one individual unit, and the entries a second unit is most likely to disagree with are flagged where they are recorded.

**Selecting an effect type now loads that type's own power-on parameters**, the way the hardware does, instead of leaving the block at zero and reading a zero back as "not set". A byte is therefore worth what it says — a level byte of zero attenuates rather than passing the signal at unity — and a file that writes a parameter *before* the type it belongs to loses that byte, which is also what the hardware does with it.

**The inserts behind the effect types carry the structure the measurement describes, each defaulting to exactly what it did before.** The phaser has a feedback path around its cascade and can sum with the dry signal instead of crossfading against it; the chorus and flanger have the input filter that sits ahead of their delay; the stereo delay has a damping filter inside its feedback loop, where its predecessor had a plain scalar and no damping at all, so a build loses its top a little further with every repeat rather than being filtered once on the way out; the rotary drives its two rotors at independent rates instead of at a fixed ratio, glides between speeds on an acceleration and a deceleration time constant held in seconds, and stops each rotor the measured distance short of the rate it was sent up to; the lo-fi stage holds at a frequency rather than at a count of samples, so its aperture null stays where it is set when the host changes rate. Every one of these is inert at its default, so a render that does not ask for it is unchanged — the built-in guitar rig included.

**Two parameters were read from the wrong slot, and both are fixed.** The overdrive and distortion types took their drive from the byte beside the drive byte — the one that selects which of four fixed response curves the stage uses — so the drive a file wrote was ignored outright and the amount of overdrive it got followed a number that names a curve. The guitar and bass multi-effect types read their EQ low and high gains from two fixed slots they were assumed to share with the single-effect types; a composite's parameter block is laid out per type rather than per block, so those slots belong to a different block in each multi and in none of them were they the EQ. The multis now read only the slots the measurement places, and their tone stack is a low shelf, a peaking section and a high shelf rather than the two shelves it had.

### GS variation tones

**Behaviour change, built-in synth.** A GS variation bank no longer always sounds its capital tone. GS addresses a variation by leaving the Program Change on the capital tone and putting a variation number in Bank Select MSB, and the built-in fallback synth previously read that number only in the GM2 renumbering the harpsichord uses — so a file written for a Roland module selected the plain tone for everything else. Thirty variations now reach a voice of their own: the wide and dark grands, the five electric pianos (detuned, velocity-mixed and the 60's reed), the three harpsichord registrations, the wide vibraphone and marimba, the church bell and carillon under the tubular bell, five drawbar-organ registrations, the flute and full-organ registrations under the church organ, the Italian musette accordion, the ukulele and key-off nylon guitar, the twelve-string guitar and mandolin, and the slow-attack violin. Each is voiced from its capital's own physical model rather than from a separate sample, so a variation stays in step with its capital when the capital is re-voiced.

Both numbering schemes address one voice, so a file written for GS and one written for GM2 sound the same tone rather than two near neighbours — except where the two standards disagree about which tone a number selects, in which case only the GS address resolves and the GM2 one keeps falling back. Every bank that is not one of the seventeen still resolves to its capital tone, which is what a module does for a variation it does not have, so no file loses a sound.

Bank Select LSB is decoded as the tone-map select the larger modules use, and it now decides what a part can reach. A GS module ships several generations of tone map and `GsToneMap` names them; a kit the selected map predates resolves to the Standard kit, which is the fallback a module gives it. Every variation tone voiced so far belongs to the earliest map, so the gate withholds no tone today and is carried on the tone table for the first one that does not. An unset LSB means the module's own map, which is the newest. GM2 gives the same controller a different meaning under its own bank MSBs, so the LSB is read as a map select only outside those two banks.

**Behaviour change, drum kits.** The rhythm-part program selects from all twenty-six GS drum sets rather than the nine it carried. CM-64/32L completes the ten the SC-55 map defines, and sixteen sets from the two later maps join them: Standard 2 and 3, Hip Hop, Jungle, Techno, Dance, Ethnic, Kick & Snare, Asia, Cymbal & Claps, Rhythm FX and Rhythm FX 2, along with the drum-machine kits — CR-78, TR-606, TR-707 and TR-909. Each is voiced from the same Standard kit table rather than from a table of its own, so a kit is a re-voicing of the shared percussion model and not a second copy of it. A program that selected no kit previously played Standard silently, and most of these now play their own. Four do not and are the Standard voicing by intent, because on a real module they are banks of one-shots and struck metal over an unchanged kit rather than a re-voiced kit: SFX, Rhythm FX, Rhythm FX 2 and Cymbal & Claps. Kit indices are append-only, so adding a set cannot renumber the sets already voiced.

### Snare drum voice

**Behaviour change, built-in synth.** The snare wires have a threshold that means the same thing on every piece, and both snares are re-voiced around it. The acoustic and electric snares sound different in every kit. Three other pieces that did render a rattle — the acoustic bass drum, the open conga and the open triangle, none of which has snare wires — are converted to the new threshold and also change. Every other kit piece is unchanged, including the twenty-eight whose fitted rattle settings are dropped here, because they rendered no rattle to begin with.

`wire_threshold` is read as a fraction of the swing a full-velocity strike reaches on that piece, where it was a level in resonator units that the mode count, strike position, mode ratios, mode decay, base frequency and the pitch drop all scale. One number therefore meant a wide-open gate on one piece and a welded-shut one on the next, and its accepted interval ran to 4 while the head swings around a tenth of that — 30 of the 33 kit notes carrying a fitted rattle rendered none at all. The interval is now `[0, 1]`, where 0 is always in contact and 1 is never, so the whole sweep means something everywhere. Strike velocity enters the same comparison, which is the nonlinearity a strainer is: a soft hit can now stay under the threshold for its whole length, where the modal excitation is a unit impulse and the gate could only be a constant.

`wire_decay_ms` is new and gives the wire bed its own damping, so the rattle can outlive the head that started it — which is why a snare's tail is broadband where its head tone has already gone. At 0 the rattle is the gate it was and renders bit-identically, which is what every piece without a strainer keeps.

The acoustic snare is re-fitted from the drum outward: the batter and resonant heads coupled through the enclosed air into a 160/250 Hz pair, the membrane ladder above them, the shell over the top and the wires from 1.5 kHz. It replaces a fit that had reached its numbers as one mode at 16.9 times the base over a 228 Hz noise band, with the shell and the wires both switched off. The electric snare's wires move inside the reference's own bandwidth: their corner had sat at 5661 Hz against a capture that ends at 5 kHz, so nothing done to that rattle could be measured.
...
**Chord recognition weighs the bass register when it chooses a root.** Folding twelve pitch classes into one vector leaves a chord and its relative — A and F#m share two of three tones — with nearly the same evidence and no cue for which note is the root, and that confusion was the dominant chord error. The recogniser now adds a low-register salience term to the template score: a candidate whose root the bass sounds is preferred over one whose root it does not. The term is deliberately a tie-breaker rather than an override, because the bass names the root only in root position; a weight large enough to overturn a clear chroma decision would relabel every inversion after its own bass note. It is measured as what the low register *adds* over the harmonic chroma, so on material with no bass part — where the low-register chromagram is a leakage-scaled copy of the harmonic one — the two cancel and the term decides nothing.

In the unified `analyze()` path this costs nothing: the bass-band CQT was already being computed and blended away, and its lowest two octaves are now kept. The C++ `ChordConfig` gains `bass_root_weight` (0 disables the term) and `use_bass_chroma`; both are core configuration and reach no binding, so the recogniser's behaviour changes on every surface while the two controls are spelled only in C++. The reported `confidence` is unchanged in meaning — it stays the selected template's plain chroma correlation, so the N.C. threshold keeps comparing like with like.

**The chord vocabulary gains eight qualities.** Sixth chords (`6`, `m6`), the minor-major seventh (`mM7`), `7sus4`, and the dominant extensions `11`, `13`, `7b9` and `7#9`. They are searched whenever the full template set is enabled — `useTriadsOnly` off — which every surface already exposes, so no new option is needed to reach them. Three of them are anagrams of a commoner quality rooted elsewhere: a `maj6` spells the `m7` a minor third below, a `min6` the `m7b5` below it, and a `7sus4` the `sus2add4` a fourth below. Nothing in a chromagram can separate those pairs, so the established reading stays the default and only bass evidence promotes the sixth. A related fix: the correlation bonus that confirms a chord's fifth now looks for the chord's own fifth rather than always for a perfect one, which a diminished or augmented chord could never collect.

**The chord HMM reads the quality standing on a cadence, not only its root motion.** `V7 -> I` and `v -> i` are the same two scale degrees, and the transition model scored them identically — discarding the strongest harmonic cue there is after the template had already measured it. Cadential motion is now graded: a dominant seventh resolving, a cadence spelled with the expected qualities, and cadential motion carrying an unexpected quality each get their own transition weight, with the last floored at the ordinary related-transition weight so grading can withdraw a bonus but never impose a penalty. The C++ `ChordHmmConfig` gains `dominant_cadential_transition_logp` and `weak_cadential_transition_logp`, which like the rest of that struct are core-only.

**Key confidence is now a distribution over the candidates that were scored.** The previous formula blended a rescaled correlation with a distinctiveness term that clipped at a fixed gap, so decisive material saturated at 1.0 and stayed there however close the runner-up was — both of two real measurements reported 100%. `Key.confidence` is a softmax over the candidate correlations instead: it falls as the runner-up closes in, two keys that split the same evidence each report about half, and a share of twenty-four candidates cannot reach 1. On silence, where nothing stands out, every candidate reports the same small share rather than a mid-range score. Every candidate's confidence is now on one scale — previously the field meant one thing at the top of the list and another everywhere else.

**It is still not a measured accuracy**, and this is called out at every surface's `Key` type. It says how decisively the chroma picked one candidate out of the set, not how often that pick is right, and nothing here has been calibrated against annotated recordings. The analyzer's own front-end selection therefore compares a separate `evidence_score`, which keeps the previous formula — half the rescaled correlation of the winner, half its margin over the runner-up clipped at a fixed gap — precisely because that blend does not move when the candidate set widens. It is core-only and no surface reports it.

**The five modal key profiles are documented as a construction.** The major and minor profiles are published ones, each derived from listening data or a corpus; the Dorian, Phrygian, Lydian, Mixolydian and Locrian profiles were unexplained hand-tuned numbers. They are now built from six named weights whose roles are stated — tonic, fifth, third, the degree that names the mode, the remaining scale tones, and everything outside the scale — and `get_mode_profile` says plainly that no probe-tone study or annotated corpus stands behind them. A new test validates each profile against a scale histogram built independently of it, replacing a case that fed each profile back to itself.


**Analysis takes a tuning offset.** `analyze`, `analyzeWithProgress`, `detectChords` and `chordFunctionalAnalysis` accept `tuning`, in fractions of a chroma bin as `estimateTuning` returns it, and shift every chroma they build by it. A recording off concert pitch read in the wrong key with no way to correct it short of resampling: a C-major loop 45 cents flat read as E major, and reads as C major with its own estimate passed back in. `detectKey` and `chroma` keep their shipped signatures; tuning reaches key detection through `analyze`.

**Each `analyze` chord carries a Roman numeral** relative to the detected key, `romanNumeral` in the result, empty for N.C. `chordFunctionalAnalysis` runs its own chord detection, so its labels could not be placed against the chords `analyze` returns.

**`analyzeWithProgress` takes the `analyze` options**, through a new C entry point `sonare_analyze_json_ex_with_progress`, so a caller reporting progress no longer has to give up the tempo curve or any other option.
### Song structure

**The analyzer no longer invents structure in uniform material.** Eight identical bars used to come back as a full verse/chorus form, because a novelty peak inside one continuous stretch of music split it into segments that then read as repetitions of each other. Two guards close that: adjacent segments whose chroma is indistinguishable are merged before anything is labelled, and when nearly every pair of sections counts as a repetition of every other — which is what uniform material looks like — repetition is treated as carrying no information rather than as evidence for an alternation.

A section whose musical function cannot be supported is reported as `Unknown` with the sub-threshold score that failed, instead of being named. `Unknown` therefore now covers three cases rather than two, and the enum documentation on every surface says which.

### Accuracy measurement

`tools/eval/summarize_accuracy.py` rolls a fixture run into publishable figures — key accuracy and MIREX weighted score, BPM accuracy at the 4% convention, beat and downbeat F-measure, meter accuracy, and chord recall in both a major/minor and an exact-quality vocabulary — broken down by dataset, with the fixture count beside every number. `make accuracy-report` runs it. `tools/eval/README.md` names the corpora and the procedure.

Two behaviours are the point of it. A dimension with no observations is reported as `unmeasured`, never as a score: a metric that skips unusable data points and averages the rest scores an empty set as perfect, and the fixture runner deliberately skips a row whose audio is missing, so a typo in a path otherwise looks exactly like a pass. And `--require` fails the run when a dimension that was expected to produce figures produced none.
...
**Behaviour change on every runtime, for a call that asks for NNDSVD seeding.** `decomposeStems`, and the entry point that takes an initialiser — `decomposeWithInit` on WASM, `decompose_with_init` on Python, `sonare_decompose_with_init` on the C ABI, and `decompose` itself on Node, which is the one surface carrying the argument on the plain call — return different factors for the same input when `init` is `"nndsvd"`. That seeding step — the starting point the multiplicative updates are given — now solves its singular value decomposition in double precision rather than single. `"random"` is the default on every surface and is unchanged, so a caller that never named an initialiser gets the factors it got before.
...
A caller who has stored factors seeded this way, or who compares such a stem render against one from an earlier release, will see them differ. Shapes, non-negativity and reconstruction quality are unaffected.

**Several channels can be decomposed into components that line up across them.** `sonare_decompose_stems_linked` takes planar channels, factorises one spectrogram built from their mean magnitude, and applies each component's single soft mask to every channel's own complex spectrogram. Component `k` is therefore the same source on every channel, the stereo image survives, and the components still sum back to each input channel. Decomposing each channel separately gives no such correspondence, because the factorisation numbers its components independently. One channel gives exactly what `decomposeStems` gives. Up to 64 channels are accepted. `decomposeStemsLinked` takes a request object on Node and WASM, `decompose_stems_linked` keyword arguments on Python.

The WebAssembly module is 40 kB smaller as a side effect, 15 kB of it compressed: this was the library's only single-precision SVD, and the code for it is no longer linked. A single-precision LDLT remains, in the LPC solver.
...
`sonare::sonare` is the target to link. It is an aggregate over whatever the installation was built with, so a consumer does not have to work out which of nineteen archives its call needs or what order they go in. The individual subsystems are exported too — `sonare::core`, `sonare::rt`, `sonare::mastering`, `sonare::engine` and the rest — for a link line that wants to stay narrow, and `find_package(sonare REQUIRED COMPONENTS mixing)` fails at configure time against an installation built with that subsystem off. The same namespaced names exist in an `add_subdirectory()` build, so moving between the two changes nothing but how the package was obtained.

Headers land in two places, matching how they are already written. The C ABI keeps its namespaced spelling at `<sonare/sonare_c.h>`. The C++ tree, whose headers include each other by their path relative to `src/`, installs whole under `include/sonare/cpp`, so both `#include "sonare.h"` and `#include <sonare/cpp/sonare.h>` resolve — and generic paths like `core/audio.h` stay out of the consumer's include root. Eigen is not a usage requirement: no installed header includes it, so a consumer needs nothing but a C++17 compiler. A `sonare.pc` is installed for the shared build, which is the configuration pkg-config can describe honestly.
...
**A mixer scene has a machine-readable contract.** `schemas/mixer-scene.schema.json` describes the document `sonare mix --scene` reads, `suggestMixScene` returns and `Mixer.fromSceneJson` builds a graph from, so a consumer without the TypeScript types — a validator, a non-TS host — can check one before handing it over. It ships in the Python wheel and sdist beside the two voice-changer schemas, and as the npm exports subpath `./schemas/mixer-scene.schema.json`. Additive: no existing entry moves and no dependency is added, but an exports subpath is a public path once published.

`additionalProperties` is false throughout, which is the point for a generated document — the parser ignores an unknown key, so a misspelt `faderDb` is a fader silently left at 0 rather than an error. Two things the schema deliberately does not describe. The parser also accepts a legacy snake_case spelling of most keys, which the writer never emits, and admitting them would let a document validate in a shape nothing produces. And a scene reaches the mixer through the C++ parser, which refuses values no schema can express: an unknown insert processor, a send naming a bus that is not in the document, a connection whose endpoints do not exist. Validating here narrows what reaches that parser; it does not replace it.

**The WebAssembly package is larger than the last release, and carries two size reductions.** The net figure first, because it is the one a browser pays: measured against the published 1.7.2 binaries, the full module goes from 4.14 MB to 5.20 MB raw and from 1.40 MB to 1.73 MB compressed, so a page that loads it transfers about 330 kB more. That is what the mixing assistant, the stem separator, the thirty GS variation voices and the re-fitted piano cost. The analysis-only module goes the other way where it counts: 964 kB to 1.00 MB raw, but 380 kB to 374 kB compressed, so its download is smaller than 1.7.2's. The two reductions below are what came *off* during the cycle, not net changes.

The first reduction takes 152 kB off the full module and 145 kB off the analysis one, about 43 kB off each compressed — a tenth of the analysis bundle's download. Almost all of it is one class of weight: constructing any `std::ostringstream` or `std::istringstream` instantiates the whole `std::locale` facet set, so the handful of call sites that formatted a number through a stream were enough to pull that set into the module, and the C library's number scanner and its 128-bit long-double arithmetic behind it. Numbers are now formatted and parsed without a stream. The text is unchanged — libc++'s own stream formatting reaches `snprintf` for this, so matching its format string matches its output byte for byte — and the decimal separator is reconciled with `LC_NUMERIC` directly, so a host running under a comma-decimal locale still reads and writes `1.5`.

**One JSON parsing behaviour changes with it.** A number too small to be a normal `double` — a subnormal, or one that underflows to zero — is now accepted and correctly rounded, where it was previously rejected as an invalid number; a document this library had itself written could contain one and fail to parse back. A number that overflows to infinity is still rejected, because a non-finite value has no JSON representation and would serialize back out as `null`.

**The second reduction was measured after the features above were in.** It takes another 157 kB off the full module and 33 kB off the analysis one; compressed, 25 kB and 8.5 kB. `bindings/wasm/wasm-size-budget.json` is rebased onto the result, so the recorded ceiling describes what this release ships rather than what a mid-cycle tree measured.

Most of that is the link line. The DSP objects are compiled for speed, so optimizing the *linked module* for size changes only the level the whole module is rewritten at, and measuring it rather than assuming settled the question it had been left open on: the intersample-peak limiter and the streaming phase vocoder both run about 2.5% faster that way, so there was no speed being traded for the 115 kB.

The rest is duplication in the mastering bindings. Eight entry points each rebuilt the same JavaScript result object out of the two result types the chain and `masterAudio` share, and four of them each installed the same progress and cancel callbacks; those have one owner each now, with the same fields in the same order.

**One WebAssembly behaviour changes with it.** The embind layer carried a second reader for the nested chain configuration, beside the path that hands the facade's flattened parameters to the core's own parameter parser. It had stopped following what it claimed to accept — `repair.declip`, `repair.decrackle` and `repair.dehum` were absent from it, and a `denoise` object was read as the deprecated boolean shorthand, so the mode, noise estimator and reduction inside it were dropped while the pass still ran. It is gone, and a configuration object that is not the flattened envelope is refused by name rather than half-read. Nothing in the package is affected: `masteringChain` and `masterAudio` flatten a nested `MasteringChainConfig` before it crosses into the module, exactly as before. A caller that instantiates the module itself and passes a nested object straight to it now gets an error instead of a partially applied chain.

### Type declarations

**The WebAssembly package ships one declaration file per source module**, where 1.7.2 shipped one rolled-up declaration per entry point. Imports through the package entry points resolve exactly as before. The `dist/` declaration tree is self-contained and resolves nothing from `src/`, so a host that vendors `dist/` alone keeps every type. Before, an unresolved emscripten module declaration turned the types behind it into `any` under `skipLibCheck`, with no error.

`EngineMidiEvent.renderFrame` is optional on Node and WASM and defaults to 0, which the native readers already did. Code that constructs the type is unaffected. Code that reads the field now sees `number | undefined`. On Python, `render_frame` stays a required argument.

### Capability catalog

**Every parameter in the catalog now carries its type, its default and the range construction accepts.** All 1,130-odd parameters across the 88 named processors reported `null` for `default`, `min` and `max` alike, which is the one thing a host needs to build a control surface from the catalog. Of the 1,147 there are now, none does: every one publishes a type and a default, and a `null` bound means what the next paragraph says it means rather than that nothing was measured. `sonare_mastering_insert_param_info` and `sonare_capability_catalog_json` now answer with real values, as do `masteringInsertParamInfo` / `capabilityCatalog` on Node and WASM and `mastering_insert_param_info` / `capability_catalog` on Python. The Node and WASM `MasteringInsertParamInfo` types declared only `name` / `id` / `rtSafe` / `unit?`; they now declare the whole descriptor, and `unit` is `string | null` rather than optional.

Both values are derived from the code rather than written down. The default is the config struct's own field initializer, recorded as each builder falls back to it, so a field whose initializer changes cannot leave a stale number in the catalog. The range is *measured*: candidate values go through the same construction path a caller uses and the catalog reports the interval validation accepted. A parameter reports `null` on both bounds in three cases, and they are worth telling apart: it has no validation, the construction path never reads its key, or no value in the probed window builds at all. In each the catalog is stating that it knows of no limit, not that a limit is unknown — but only the first means the processor will take anything.
...
**`saturation.multibandExciter` publishes one set of controls per band, and an automation id now moves one band instead of all of them.** Its descriptors named `frequencyHz`, `driveDb`, `amount`, `q` and `evenOddMix`, while its builder reads those same five fields under the `band0.` / `band1.` / `band2.` prefix every other band-splitting processor uses — so a host read five controls under keys construction does not accept, with no type, no default and no range behind any of them. It publishes fifteen now, five per band, keyed and numbered the way `multiband.compressor`, `multiband.expander`, `multiband.limiter` and `multiband.saturation` already were: parameter `p` of band `b` is id `b * 5 + p`. **Automation by id has to be renumbered.** Id 1 was drive on every band at once and is now drive on band 0; the ids above 4 were rejected and are now the second and third bands.

It was also the one band-splitting processor that accepted only the split it was built with: a crossover naming any number of cutoffs other than the default two was refused as a band-count mismatch, where the other six resize to match. It resizes too, so a four-band configuration builds and the fourth band reads its own `band3.` fields.

**The catalog describes every parameter construction reads, not only the ones that can be automated.** `sonare_mastering_insert_param_info` used to list an insert's automation targets and nothing else, so a setting that takes effect only when the insert is built had no entry at all. The hard and soft clippers, the waveshaper, the exciter and the presence enhancer select their 4x oversampling path through such a setting, `aliasing`, and a host had no way to find it or its values. Those settings now follow the automation targets in the same array, sorted by name, with a null `id` and `rtSafe` false. The catalog goes from 1,154 parameter entries to 4,512. **Code that reads `id` from every entry has to skip the null ones**: the Node and WASM types say `id: number | null`, which turns the mistake into a compile error there; Python has no such check.

A parameter that accepts only certain values lists them in a new `choices` field as `{name, value}` pairs, and each value is included only if the insert actually builds with it. An enum-typed setting reports `type: "enum"`: the exciter's `aliasing` lists `none` and `oversample4x`, because the exciter refuses the two ADAA modes. A whole-number setting whose accepted values have gaps also gets `choices` while its type stays `"number"`, for example the tube stage's `oversampleFactor` of 1, 2, 4 or 8. When `choices` is present, `min` and `max` are null. `dynamics.compressor`'s `detector` was already listed and changes type from `"number"` to `"enum"`. Settings that carry a string or an array (an embedded impulse response, a per-band list) report `type: "string"` or `"array"` and have no default or bounds. A whole-number setting whose accepted values have gaps and continue past the measured window, such as the odd-only linear-phase `kernelSize`, reports its minimum and a null maximum.

**The band keys of five equalizers are listed, and both command-line front-ends accept them.** `eq.minimumPhase`, `eq.linearPhase`, `eq.dynamic`, `eq.midSide` and `multiband.dynamicEq` read a band's fields only once the band is switched on, so building an insert with no parameters never read them. As a result `sonare_mastering_insert_param_names` left out `band0.type`, `band0.q` and the rest, although its documentation says band keys are listed. The native and Python CLIs check `--params` against that list, so both refused `eq.linearPhase --params '{"band0.frequencyHz":1000,"band0.type":1}'` as an unknown key. They accept it now. Bounds and choices for a band key are measured with its band switched on, so several band `min` values on those five inserts change from null to a number (`q` becomes 0, for example). A band key sent without the band's `frequencyHz` is still ignored by the processor.

**Behaviour change: an enum value no enumerator declares is refused.** The EQ band type, stereo placement, phase mode and coefficient mode, the amp topology, and the amp, cabinet, microphone and power-tube models used to replace an unknown value with their first enumerator. Other selectors let an undeclared value through to the processor: the crossover slope and mode, the linear-phase resolution, the Pultec component model, the cut-filter slopes, the compressor detector, the waveshaper curve, the bitcrusher's dither type and quantizer mode, the phaser mix mode and the chorus and flanger pre-filter. All of them now refuse such a value by key name. This applies to inserts, to offline processors and to restoring a chain from its flat parameters. Declared values behave exactly as before.

**A host can ask what latency a configured insert will report.** `sonare_mastering_insert_timing` builds an insert from flat parameters, prepares it at a given sample rate and returns the latency and tail it reports, so a slot that switches on oversampling or a linear-phase crossover can be compensated without building the insert on the host's side. `masteringInsertTiming(name, params, sampleRate)` on Node and WASM and `mastering_insert_timing(name, params, sample_rate)` on Python return `{ latencySamples, tailSamples }`. A key the insert does not read is refused rather than ignored, because the answer would otherwise describe a configuration the caller did not ask for. The catalog's `latencySamples` and `tailSamples` are this query at default parameters and 48 kHz. No insert's latency depends on the block size it is prepared at, and a test now checks that.
...
`NOTICE` carries the attribution for the FFT implementations bundled inside `third_party/r8brain/fft/`. The vendored r8brain licence covers its own author only; the bundled Ooura FFT (the one r8brain's own resampler uses, since neither `R8B_PFFFT` nor `R8B_IPP` is defined), PFFFT and PFFFT DOUBLE, their FFTPACK/NCAR ancestry, and the Apache-2.0 AVX-to-NEON layer each keep their own copyright notice, and the BSD-family terms require those to be reproduced.

### Performance

**The FFT runs on SIMD kernels.** PFFFT serves every transform length it can factor and KissFFT keeps the rest, so nothing about the accepted `n_fft` range changes. Measured with and without the change on one tree, over the benchmark fixture (73 s resampled to 22.05 kHz, Apple silicon), a 2048-point STFT drops from 13.6 ms to 6.0 ms, and everything built on the STFT inherits it: chroma 15.0 → 7.5 ms, spectral centroid 19.2 → 10.8 ms, mel 22.7 → 15.2 ms, MFCC 23.5 → 16.1 ms, onset strength 23.1 → 15.5 ms, HPSS 88.2 → 61.9 ms, pYIN 425 → 337 ms, and the whole `analyze()` pipeline 963 → 611 ms. The measurements that reuse an already-computed spectrogram are unchanged, which is the control: nothing outside the transform moved. Results move by floating-point rounding only — under a tenth of a decibel below the noise floor of any meter, though enough to reshuffle a quantized golden hash, and four mastering preset goldens are rebaselined accordingly -- `aiMusic`, `speech`, `podcast` and `audiobook`. `-DSONARE_USE_PFFFT=OFF` selects the previous KissFFT-only build, and the WebAssembly configuration uses it by default. Carrying both FFTs there costs the analysis-only module more than its size budget allows, and buys 8.3% on an STFT-heavy pipeline rather than anything like the figures above, because a phase vocoder spends most of its time outside the transform. The browser modules keep KissFFT alone.

**Offline LUFS no longer scales its working set with the clip length.** The K-weighted signal was materialized whole, one `double` per sample — 1.4 GB of scratch for an hour of 48 kHz audio, in a single buffer the channels took turns in. It is now filtered in chunks into a sliding window holding only what the earliest unfinished gating block still needs, which is bounded by the 3 s short-term window regardless of how long the input is. Measured over an hour of 48 kHz mono, peak resident memory falls from 2.09 GB to 706 MB — the remainder is the caller's own input buffer. The filter state carries across chunks and every block is still summed in one pass over its own window, so the reported loudness is bit-for-bit what it was.
...
- A mixer can gain a channel strip after it is built. `addStrip` on Node and WASM, `add_strip` on Python. The strip set was fixed at whatever the scene document declared, while buses and VCA groups could be added and removed on a live mixer and the C entry point had supported strips all along — so reaching a new strip meant serializing the scene, editing the JSON and constructing a *different* mixer, which drops the automation position and queued events `compile()` exists to preserve. The optional metering argument (`enabled` / `lufs` / `truePeak` / `truePeakOversample`) is the strip's `metering` object from scene JSON under the same names, and omitting it gives the full configuration a scene strip gets by default. As with a bus, the graph is marked dirty and `compile()` rebuilds it.
- **Behaviour change.** A mixer scene's processor `params` is an object in the JSON document instead of a JSON string nested inside it. Every consumer had to parse twice, a malformed blob travelled past scene loading to fail later at insert construction, and the document could not be diffed or grepped per parameter — the cross-surface test harness carried a helper whose only job was to see inside the string. Reading accepts both forms, so every project file saved so far still loads; writing emits the object, so a document produced now needs a reader from this version. `MixSceneInsert.params` on Node and WASM is typed as the record it always was.
- **Behaviour change.** A synth patch asking for the vocal body gets one. `clamp_synth_patch` validated the body type against a literal upper bound of 4, which was the last member when it was written and stopped being so when `kVocal` was added at 5, so every vocal body was silently reset to none — on all four surfaces, since the C ABI, the type tables and the resonator itself had all carried `SONARE_SYNTH_BODY_VOCAL` correctly the whole time. A caller passing it saw no error and got an unresonated voice. Two GM fallback voices were built around the mechanism and are audibly different now: program 85 (Lead 6, voice), whose oscillator exists to feed the formant bank, and program 91 (Pad 4, choir). No other patch in the bank requests it, and the sixteen lead and pad programs stay inside their 3 dB level band. The bound is now the enum's last member, so the next body type added cannot repeat it.
...
- **Behaviour change, WASM.** `detectBoundaries` refuses a wrong-typed option by name instead of coercing it. Its ten options were read through the coercing readers, so a numeric string or a one-element array arrived as the number it spells and any string at all arrived as `true` — `useChroma: 'false'` turned "disable this stream" into enabling it, and the caller got a plausible result with no way to tell. The addon already refused the same inputs, so one call was a `TypeError` on one runtime and a silent reinterpretation on the other. Only these ten fields move; the coercing readers stay as they are elsewhere, where tightening them would change a published contract.
- `Boundary`, `BoundaryOptions` and `BoundaryResult` are exported from the Node package entry. They were declared in the analysis type module but never re-exported, so a TypeScript consumer could call `detectBoundaries` and had no name for what it returns.
- **Behaviour change, CLI.** `sonare boundaries` reaches the whole detector. `--absolute-threshold`, `--n-mfcc`, `--n-chroma`, `--no-mfcc` and `--no-chroma` are new, and `--min-distance` is now `--peak-distance`, the name the three library surfaces already use. The missing one that mattered is the absolute threshold: `--threshold` is relative to the novelty curve's own maximum and so cannot ask whether anything changed at all, which left stationary material segmenting every few seconds with no option to stop it. Each size is now refused at zero and the two feature streams cannot both be disabled, matching what the C entry point has always reported. Scripts passing `--min-distance` need the new spelling.
- `detectBoundaries` on WASM reaches the library's own defaults. The facade restated all ten as TypeScript literals before calling into the module, so the values the core publishes were unreachable from the npm package and a change to one of them would have moved Python, Node and the CLI while leaving WASM on the old number, silently and with no test able to see it. Omitted options now arrive absent and take `BoundaryConfig`'s own initializers. An option explicitly set to `null` reads as absent on this surface too, which is what the addon has always done.
- `detect_boundaries` on Python is held to the same defaults: its ten keyword literals are now compared against `sonare_boundary_options_default()`, the entry point the addon reads live and nothing on this surface called.
- **Behaviour change, CLI.** `sonare melody --hop-length 0` no longer hangs. The melody contour frames the signal itself instead of going through the STFT, so the hop guard that covers every other command did not cover this one, and a hop of zero left the frame loop unable to advance — the command consumed memory until it was killed. It is refused now, along with a non-positive `--threshold` or `--fmin` and an `--fmax` at or below `--fmin`.
- **Behaviour change, CLI.** `chords`, `dynamics`, `rhythm`, `sections` and `key` refuse the option values the library refuses through every other surface. The command-line front-end builds the analyzers directly, and most of them check only that the audio is non-empty, so a zero `--smoothing-window`, a negative `--hmm-beam-width`, a non-positive `--window-sec`, an inverted `--bpm-min`/`--bpm-max` pair, a negative `--min-duration`, or a `--hop-length` below 16 together with `--use-hpss` were all accepted and answered with a plausible result. Each is now reported as an invalid parameter, which is the code the Python front-end already returned for the same values.
- **Behaviour change.** A comp seam can crossfade instead of stepping. Comping splices takes together, and two takes are different signals, so a butt join steps the waveform at every splice unless the seam happens to land on a shared zero crossing — measured on a pair of takes held a constant apart, the join moved the full distance between them in one sample. `crossfade_ppq` on a comp segment fades it in over the part before it, taken from before the seam so the switch completes where the segment starts and the clip's length does not move; it is equal-power, so a correlated seam holds its level. The same pair crossfaded over a quarter of a beat moves in steps 154 times smaller. The field defaults to 0, which is the butt join every existing comp already renders, so no project changes without being edited. A segment at position 0 has nothing in front of it and must leave the field at 0, and a fade cannot exceed either the segment's own length or the preceding part's. `ClipSchedule` gained the matching `seam_fade_in_samples` / `seam_fade_out_samples`, measured against a fragment's own span rather than the clip-wide fade reference. The loop seam already had a crossfade; the comp seam is the sibling that did not.
...
- **Behaviour change, C ABI.** A zero `reference_absorption` in `SonareRoomEstimateConfig` now selects the library default (0.15) like every other float in that struct, instead of being taken literally. A C caller following the documented `SonareRoomEstimateConfig cfg = {};` idiom had its zero clamped to the analyzer's 0.01 floor, and the Eyring volume goes as the cube of the absorption prior — a 70 m³ room was reported as 0.02 m³ with full confidence, and feeding that estimate back into `sonare_synthesize_rir` gave an unrelated reverb. Node, Python and WASM already passed 0.15 and are unaffected. A caller that genuinely wants a near-rigid prior should request 0.01 explicitly.
- **Behaviour change.** `bandScattering` applies to whichever wall material the absorption precedence selected, rather than only alongside `bandAbsorption`. A room built from a scalar absorption or from a `materialPreset` accepted the scattering array, validated it, and then discarded it without an error or a diagnostic, so a request for rough walls produced a perfectly specular room with a wrong mixing time and early/late balance. This affects the C ABI, Node, WASM and the `effects.acoustic.roomMorph` insert; the Python docstring was the only one that described the old dependency, and all four surfaces' docs now state the new rule. A scattering array supplied with a preset now changes the render.
- **Behaviour change.** `effects.reverb.room` and `effects.acoustic.roomMorph` reject an `absorption` outside [0, 1] instead of clamping it. The offline `synthesizeRir` facade already returned `InvalidParameter` for the same value, and the inserts' own per-band array path already threw — only the scalar silently built a different room than the caller asked for. Constructing a `RoomReverb` directly refuses it too: the shoebox builder clamps rather than validates, so an out-of-range coefficient reached the geometry check already inside the range and was accepted as a different room. Both command-line front-ends refuse `--absorption` outside [0, 1] on `synthesize-rir` and `room-morph`, which is the last path that reached that builder unchecked.
- **Behaviour change, C ABI.** `sonare_project_load_soundfont` returns `SONARE_ERROR_INVALID_FORMAT` for a malformed SF2 and for one over the resource budget, matching `sonare_engine_load_soundfont` and the rule the project MIDI import already documents. `SONARE_ERROR_INVALID_PARAMETER` is now reserved for a NULL project/buffer or a zero length, so a caller can tell a broken file from a broken call. The Python CLI's exit code for a corrupt SoundFont loaded through a project moves from 3 to 5.
- The amp-sim insert reports a loaded or generated cabinet IR in its decay tail, so an offline bounce no longer truncates up to the IR's length of cabinet decay — 40 ms at 48 kHz on the maximum IR, silently. `tail_samples()` previously answered only with the second mic's path-length delay, which is zero unless a mic pair is configured.
- The amp-sim insert honours the channel-aware `prepare` overload, so an offline mono or stereo render reserves per-channel state for the channels it will actually process. Each channel owns a cab-IR ring, a Doppler line and two mic delay lines; preparing the realtime maximum for a mono bounce cost 84 MB against the 2.6 MB the equivalent `saturation.tube` render uses, and risked an allocation failure in a large WASM mixing graph. Realtime callers using the two-argument `prepare` are unchanged.
- **Behaviour change.** The FIR realization of a match EQ returns to unity outside the matched band instead of extending the curve's edge gain to DC and Nyquist. The curve is defined only over `[minFrequencyHz, maxFrequencyHz]`, and the parametric realization places no band outside it, so the two produced different low ends from the same match; a thin source against a bass-heavy reference had `match.applyMatchEq` building up to 12 dB at DC, adding an offset and subsonic energy that ate the headroom of everything downstream. The gain now tapers over one octave beyond each edge, narrowed on the high side when Nyquist is nearer than that.
- `stereo.monoCompatCheckLogBands` measures each band over its whole `[lowHz, highHz)` interval instead of at a single frequency at the band's log centre. Two components sharing a band — one in phase, one anti-phase — read as fully correlated because only the one nearest the centre was seen; they now read as the cancelling pair they are. The measurement is also a shared set of transforms rather than one full-length DFT per band, so a thirty-band split over a long buffer no longer costs thirty passes over every sample.
- The offline mono mastering chain releases each track-length measurement copy as it reduces it, matching the stereo path. It previously held the input alive across every stage, so a 60 s 96 kHz mono render grew the resident set by 133 MB where it now grows by 89 MB.
- `MasteringChain`'s progress callback documents what it counts: the fraction of enabled DSP stages, so it reaches 1.0 as the last stage returns and the trailing output measurement, spectrum and band delta report nothing. The callback sequence is unchanged; the previous "0.0..1.0 across the whole chain" wording was not.
- **Behaviour change on every runtime.** `roomMorph` reports what the target-room synthesis had to change to make the morph, instead of dropping it, and **its return type changes on every surface**. The morph builds its target room with the same code `synthesizeRir` uses, which reports an image-source order reduced to the safe maximum, a tail cut against `maxSeconds`, and a request that produced no diffuse tail — each of which says the morph went through a room other than the one asked for. All of them were discarded, so the two cases were indistinguishable. Every surface now returns `{ audio, sampleRate, diagnostics }` — `RoomMorphResult` on Node, WASM and Python — shaped like `synthesizeRir`'s result but with no `hasError` counterpart, because an unusable morph throws rather than reporting. Both command-line front-ends print them. A caller reading the samples needs `.audio`.
- **Behaviour change, C ABI and Python.** Diagnostics reach a caller with their code, message and severity intact instead of flattened into one line. `sonare_last_warning_message()` joins every warning with `"; "`, which cannot be split back out when a message holds the separator and drops the severity entirely — so Python, the one surface that reads the C ABI rather than the core, could not rebuild the array Node and WASM hand back. `sonare_last_diagnostic_count()` and its `_code` / `_message` / `_severity` accessors publish the same entries unflattened, on the same thread-local contract as the warning string and cleared alongside it, and `synthesizeRir` and `roomMorph` publish there. Python's `RirResult.warning_message` and `RoomMorphResult.warning_message` are replaced by `diagnostics`, a list of `RirDiagnostic` matching the Node and WASM field for field; the joined string stays available through `sonare_last_warning_message()` for callers written against it. The Python front-end prints one line per warning, which is what the native binary already printed.
- **Behaviour change.** The Mel band count is bounded and an oversized request is refused by name instead of faulting. The filterbank's extent is the band count times the FFT bin count, computed as an `int`: 4190212 bands over a 2048-point FFT reached 4294967300, truncated to 4, and the filling loop then wrote past a four-element buffer. The extent is computed in `size_t` now, and the band count carries a ceiling of 4096 — 32 times the default, so it sits above every consumer's useful maximum instead of binding one. It is reachable from every surface, since the C entry point passes `nMels` straight through and each binding checked only that it was positive.
- **Behaviour change on every runtime.** `griffinLim` holds its analysis geometry to the overlap-add rule its sibling reconstructing paths already check. Past a hop of `nFft / 2` the windows stop summing to a constant and the reconstruction comes back amplitude-modulated at the frame rate: on one 2048-point signal the output's peak-to-trough envelope ratio grows from 1.95 at the boundary to 15.6 at a hop of the whole window, where it also clips. Nothing refused it — the C entry point checked only that the hop was positive — so the call returned a plausible-looking buffer with no error anywhere. A hop past half the window is now an invalid parameter on all four surfaces. The iteration count gains a matching ceiling of 256 in the core, which is a backstop rather than a new limit: the C entry point and the WebAssembly wrapper already enforced the same number, so it closes the gap for a C++ caller reaching the core directly. Every shipped surface requires a positive count, so the range a binding accepts is `[1, 256]`; the core itself takes 0, which is the random-phase seed reconstructed once.
- **Behaviour change.** `sonare_note_move` refuses a target the buffer has no room for instead of writing the note nowhere. The span the note vacated is ramped out whatever the target is, so a target past the end erased the note and pasted it nowhere while the call reported success. A target inside the buffer is accepted and keeps the part that fits — moving a note to 10800 of an 11025-sample buffer truncates it rather than refusing — and only a target with no room at all is an invalid parameter. Reachable from every surface and from `note-move` on both command-line front-ends.
- **Behaviour change.** `tempogram`, `fourierTempogram` and `plp` bound their window length, which was previously unbounded. The autocorrelation is quadratic in the window and linear in the frame count, so a large window does not finish rather than failing: over one second of audio at the default geometry, the default 384-frame window takes 0.07 s, 4096 frames take 0.44 s, and the new ceiling of 12288 takes 4.0 s. That ceiling is 32 times the default and is a lag near 285 seconds, so it sits above every consumer's useful maximum instead of binding one — it is there to stop a window that never returns, not to narrow a working call.
- **Behaviour change, Python.** A synthesis seed the C field cannot hold is refused rather than folded into it. `synthesize_rir` and `room_morph` take a `uint32` seed at the C ABI, and Python masked an out-of-range value into range — which made `-1` and the library default indistinguishable under a successful call, so two different requests returned the same tail with nothing to tell them apart. Node and WASM already refused it. A script passing a negative or oversized seed now raises.
- **Behaviour change, CLI.** `--seed` on `synthesize-rir` and `room-morph` reaches the whole field on the native front-end. It was parsed as a C `int`, so the top half of the `uint32` range the C ABI accepts could not be spelled — including the value the refusal's own message quoted as the maximum. Both front-ends now accept `[0, 4294967295]` and publish that range.
- **Behaviour change, CLI.** `pitch-shift`, `time-stretch`, `mel`, `cqt`, `vqt`, `tone`, `chirp`, `clicks`, `synthesize-rir` and `room-morph` refuse the values the library refuses through every other surface. The spectral backend repaired a non-positive `--n-fft` or `--hop-length` into the librosa defaults rather than refusing it; a negative `--fmin` or `--fmax` was substituted with the same default an omitted flag selects, which left a typo indistinguishable from an omission; the generators accepted any positive `--sr`, including rates this library's own decoder cannot read back, so the file they wrote was unreadable by every command that could have consumed it; and `--absorption` reached a builder that clamps rather than validates. Each is now an invalid parameter. The Python front-end already refused every one of them through the library, and now publishes the same accepted range as the native registry, so the two cannot narrow apart without the cross-surface contract saying so. `cqt`, `vqt`, `tone`, `chirp` and `clicks` exist only on the native front-end.
- The command line reports the formant factor and the constant-Q gamma the result was actually made with. `voice-change` echoed the factor it was given, which the warp first clamps into its own range and then folds through the dry/wet amount before applying; `vqt` echoed a negative gamma as the sentinel it is rather than the ERB-derived value the bandwidths were built from. Both resolutions are now published by the code that applies them, so the reported value and the applied one cannot drift apart. A NaN factor comes back a NaN and is refused rather than resolved.
- **Behaviour change.** An explicit `gain: 0` on a synth patch renders silence. The native synth and the SoundFont player replaced any non-positive gain with 0.5, so zero was the one level a caller could not reach, although the patch documentation says an explicit zero overrides. Only a negative or non-finite gain falls back now.
- **Behaviour change.** A synth mod routing whose source or destination is `none` is refused with `InvalidParameter` on every surface, as a controller binding with axis `none` already is. It used to be kept as a silent no-op slot, so a routing that reached nothing looked accepted. No shipped preset or GM fallback patch carries one.
- **Behaviour change.** `beatLocalBpm` is a continuous value. The curve was decoded on a 64-state grid about 2.9% apart, so a steady track read up to 1.45% off its tempo and a ramp came out as a staircase. The smoothing problem is now solved exactly, with the same weights, and a steady tempo reads within about 1% at every beat.
- **Behaviour change.** A section floor below the section length no longer removes boundaries. `minSectionSec` was also the boundary detector's peak spacing, and short spans were absorbed by position, so a floor of 6 or 8 s could collapse a track of 8 s sections to one `Unknown` span. The floor is now enforced by keeping the strongest set of boundaries whose sections all reach it, so section lists at the default floor can change too.
- **Behaviour change.** Synthesized room responses decay per band as designed, and the octave-band analysis measures them without cross-band leakage. Both sides split bands with a single Q-√2 bandpass, which let a slow low band overtake a fast high band inside the fit window, so a single-material room read its 125 Hz decay in every band. Every synthesized RIR changes, and tails sound brighter. The band just above a large RT60 step still reads long.
- **Behaviour change.** A band whose impulse response ends before its decay does is reported as NaN rather than fitted, and `synthesizeRir` warns `acoustic.rir_tail_truncated` when `maxSeconds` is shorter than the longest band's RT60. A large hall's tail is no longer placed about 90 dB too low when no reflection arrives inside the level-matching window.
- **Behaviour change.** `estimateRoom` reports volume and dimensions as NaN when no decay is measurable, as the rest of the acoustic family does, instead of 0. The CLI writes them as `null`.
- **Behaviour change.** A hosted synth's lane fader, mute and gate reach everything that synth produced for that lane. The native synth and the SoundFont player handed each lane its own dry voices, while their shared processing — the piano soundboard and sympathetic strings, the plucked-string halo, a part's insert effect, the insertion units, the reverb, chorus and delay returns, the bus drive and the DC blocker — went to the destination as a whole. A saw lead with its lane fader at -96 dB dropped by only about 18 dB, and a stem rendered by muting the other lanes carried their reverb. Each shared stage is now split across the lanes whose voices fed it: a bussed part's whole insert output follows that part, an effect return follows each voice's send level, and a body resonance follows the voices driving it. In the tests, a lane with its reverb send at 0 now carries none of another lane's tail; it carried one only 3.45 dB below its own dry level before. A lane at -96 dB loses its whole bussed insert output. The remaining shared stages have no voice of their own, so they are split by each lane's share of the dry signal: the DC blocker, the master EQ and the bus drive. That split is exact for one sounding lane and close for several. A layered instrument now renders each layer through the lanes that played it too. A render with one lane is unchanged; a mix of several lanes sums to what it did before, up to rounding.
- **Behaviour change.** Clearing an automation lane returns its target to the manual value. It used to hold whatever the lane last evaluated to. The manual value is the last one sent through `setParameter` or `setParameterSmoothed`, including one sent while the lane was still running. The result no longer depends on whether the manual value or the lane clear reaches the audio thread first, although the worklet sends them over two different channels. A target that never received a manual value stays where the lane left it. The engine remembers manual values for a fixed number of targets. A value past that limit still applies but is not restored, and telemetry reports it as the new code 21, `PARAMETER_BASE_OVERFLOW` (`ParameterBaseOverflow` in the worklet protocol).
- A time-stretched clip keeps the pitch of every channel. The splice search scored its overlap on the first channel alone and applied the result to all of them. On a stereo clip whose sides carry different material, the second channel was spliced out of phase: stretched by 1.25, a tone on the right drifted about 20 cents flat. The search now scores all channels together, and the same clip stays within a few cents on both sides. Every channel still gets the same splice point, so the stereo image holds.
- The native WASM `RealtimeEngine` methods that take a trailing render frame accept it omitted, as `-1` ("now"), as the TypeScript wrapper and the Node binding already did. Raw embind threw a BigInt conversion error.
