...
**`repair` no longer discards the repair it just made.** Declipping rebuilds the peaks a clipper cut off, so its output routinely exceeds full scale, and this command deliberately runs no limiter — the integer writer then clamped, pinning exactly the samples the repair had rescued back onto the ceiling they came from. On a fifth-clipped file the written output still had 8446 of its 8672 samples sitting at full scale, so 97 % of the reconstruction was thrown away at the last step. The output is now fitted to full scale with one gain for the whole file, which keeps the reconstructed waveform's shape, and the applied gain is reported as `output_gain_db` in `--json` and as a line in the text report. The key is always present and is `0` when the peak already fit, so a file that did not clip is written exactly as before.

**`project bounce --synth` and `midi-render --synth`, spelled bare, send the same synth patch from both front-ends.** They did not: the native CLI named the `sine` preset as the patch's base while the Python CLI sent the init patch, a 22-field difference. Nothing rendered differently, because the bare flag also turns on GM program routing and that replaces the patch at every note-on — but the two front-ends disagreed about what would happen if it ever did not. The native side now sends the init patch, which is what the empty preset name already means. No rendered output changes.
...
Bank Select LSB is decoded as the tone-map select the larger modules use, and it now decides what a part can reach. A GS module ships several generations of tone map and `GsToneMap` names them; a tone or kit the selected map predates resolves to its capital tone or to the Standard kit, which is the fallback a module gives it. An unset LSB means the module's own map, which is the newest. GM2 gives the same controller a different meaning under its own bank MSBs, so the LSB is read as a map select only outside those two banks.
...
In the unified `analyze()` path this costs nothing: the bass-band CQT was already being computed and blended away, and its lowest two octaves are now kept. `ChordConfig` gains `bassRootWeight` (0 disables the term) and `useBassChroma`. The reported `confidence` is unchanged in meaning — it stays the selected template's plain chroma correlation, so the N.C. threshold keeps comparing like with like.
...
- The amp-sim insert reports a loaded or generated cabinet IR in its decay tail, so an offline bounce no longer truncates up to the IR's length of cabinet decay — 21 ms at 48 kHz on the maximum IR, silently. `tail_samples()` previously answered only with the second mic's path-length delay, which is zero unless a mic pair is configured.
