...
- **Behaviour change on every runtime.** `roomMorph` reports what the target-room synthesis had to change to make the morph, instead of dropping it, and **its return type changes on every surface**. The morph builds its target room with the same code `synthesizeRir` uses, which reports an image-source order reduced to the safe maximum, a tail cut against `maxSeconds`, and a request that produced no diffuse tail — each of which says the morph went through a room other than the one asked for. All of them were discarded, so the two cases were indistinguishable. Node and WASM now return `{ audio, sampleRate, diagnostics }` where they returned the samples alone, shaped like `synthesizeRir`'s result but with no `hasError` counterpart, because an unusable morph throws rather than reporting; Python returns a `RoomMorphResult` carrying `audio`, `sampleRate` and `warningMessage`; the C ABI leaves the warnings in `sonare_last_warning_message()`. Both command-line front-ends print them. A caller reading the samples needs `.audio`.
