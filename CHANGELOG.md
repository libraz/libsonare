...
**The WebAssembly package is smaller.** The full module drops 152 kB to 4.98 MB and the analysis-only module 145 kB to 988 kB, back under a megabyte. Compressed, which is what a browser actually transfers, that is about 43 kB off each — 1.61 MB to 1.57 MB, and 410 kB to 367 kB, a tenth of the analysis bundle's download. Almost all of it is one class of weight: constructing any `std::ostringstream` or `std::istringstream` instantiates the whole `std::locale` facet set, so the handful of call sites that formatted a number through a stream were enough to pull that set into the module, and the C library's number scanner and its 128-bit long-double arithmetic behind it. Numbers are now formatted and parsed without a stream. The text is unchanged — libc++'s own stream formatting reaches `snprintf` for this, so matching its format string matches its output byte for byte — and the decimal separator is reconciled with `LC_NUMERIC` directly, so a host running under a comma-decimal locale still reads and writes `1.5`.
...
**And smaller again, now that the features added since have been measured.** The full module drops another 157 kB to 5.20 MB and the analysis-only module 33 kB to 999 kB; compressed, 25 kB and 8.5 kB, which puts the analysis bundle back under the size it is recorded against.
